...
USE CHEMCONS, ONLY: WRITE_CVODE_SUBSTEPS, ZETA_ARTIFICAL_MAX_LIMIT,ZETA_ARTIFICAL_MIN_LIMIT, ZETA_FIRST_STEP_DIV,&
                    MIXED_ZN_MODEL,IGNITION_ZONES,ADJUST_ENERGY_FOR_MIXING_ZN
...
REAL(EB) :: ZZ_MIXED(1:N_TRACKED_SPECIES)
...
REAL(EB) :: MW_MIXED, RHO_OUT,RHO_MIXED,T1, T2, ZETA0, ZETA_IN_MOD, TMP_IN_MOD, TMP_OUT, CHEM_TIME,&
            DT_MOD, ZETA_ARTF, ZETA_FINAL, ZETA_MAX_LIMIT, ZETA_MIN_LIMIT,FLAME_TMP,EQUIL_TMP,PRES_OUT
REAL(EB) :: DU, DUDT
...
ELSE
   IF (SIM_MODE .NE. DNS_MODE) THEN
      ...
      ELSE IF(TRIM(MIXED_ZN_MODEL) .EQ. "STOICH-PHI-KIN-EQUILTMP") THEN
         CALL CALC_STOICH_FLAME_TEMPERATURE(ZZ,TMP_IN,PRES_IN,FLAME_TMP,ZZ_UNMIXED,ZZ_MIXED,ZETA_IN_MOD)
         IF (FLAME_TMP < FINITE_RATE_MIN_TEMP) RETURN
         EQUIL_TMP = FLAME_TMP
         !USE_DTMPDT_LIMITER = .FALSE.
         !TMP_IN_MOD = MAX(MIN(FLAME_TMP,MIXING_ZN_TMP_LIMIT),TMP_IN)
         TMP_IN_MOD = MAX(TMP_IN,FLAME_TMP)
         IF (ADJUST_ENERGY_FOR_MIXING_ZN) THEN
...
IF(ZETA_IN_MOD > ONE_M_EPS) THEN
   ...
   CALL CVODE_SERIAL(CC_CHEM_TIME,ZZ,ZZ,TMP_IN_MOD,TMP_IN,EQUIL_TMP,USE_DTMPDT_LIMITER,PRES_IN,ZETA0,TAU_MIX,CELL_MASS, &
   ...
   IF (CALL_CHEM_AGAIN) THEN 
      ...
      CVODE_CALL_OPTION = 1 ! CV_NORMAL
      CUR_CFD_TIME = T_CFD ! Set current cfd time in cvode, for logging purpose.
      CALL CVODE_SERIAL(CC,ZZ,ZZ,TMP_IN_MOD,TMP_IN,EQUIL_TMP,USE_DTMPDT_LIMITER,PRES_IN,ZETA0,TAU_MIX,CELL_MASS,&
...
CUR_CFD_TIME = T_CFD ! Set current cfd time in cvode, for logging purpose.
ZZ_IN = ZZ
CALL  CVODE_SERIAL(CC,ZZ_UNMIXED,ZZ,TMP_IN_MOD,TMP_IN,EQUIL_TMP,USE_DTMPDT_LIMITER,PRES_IN,ZETA0,TAU_MIX,CELL_MASS,&
                   T1,T2,GLOBAL_ODE_REL_ERROR, ATOL,TMP_OUT, CHEM_TIME, WRITE_SUBSTEPS, CVODE_CALL_OPTION)
...
!> \param TMP_IN is the temperature
!> \param AFT is the adiabatic flame temperature (output)
SUBROUTINE CALC_STOICH_FLAME_TEMPERATURE(ZZ,TMP,PRES,TMP_OUT,ZZ_UNMIXED,ZZ_MIXED,ZETA)
   USE PHYSICAL_FUNCTIONS, ONLY: CALC_EQUIV_RATIO,GET_ENTHALPY,GET_TEMPERATURE
   USE CHEMCONS, ONLY: DILUENT_SPECIES_LIST

   REAL(EB), INTENT(IN) :: ZZ(N_TRACKED_SPECIES),TMP,PRES
   REAL(EB), INTENT(OUT) :: TMP_OUT,ZZ_MIXED(N_TRACKED_SPECIES),ZZ_UNMIXED(N_TRACKED_SPECIES),ZETA
   REAL(EB) :: ZZ_MIXED_DUMMY(N_TRACKED_SPECIES)
   REAL(EB) :: PRES_OUT

   CALL BUILD_ZZ_UNMIXED_MIXED(ZZ, ZZ_UNMIXED, ZZ_MIXED, ZETA)
   ZZ_MIXED_DUMMY(1:N_TRACKED_SPECIES)=ZZ_MIXED(1:N_TRACKED_SPECIES)
   CALL EQUILIBRATE(ZZ_MIXED_DUMMY,TMP,PRES,TMP_OUT,PRES_OUT,1)
END SUBROUTINE CALC_STOICH_FLAME_TEMPERATURE
