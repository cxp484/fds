...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   Q     = 0._EB
   CHI_R = 0._EB
   INITIAL_TMP_MIX = 0.0_EB
   FINAL_TMP_MIX = 0.0_EB
   MOD_ZETA = 0.0_EB
...
INITIAL_MIXED_ZN_TMP = 0.0_EB
FINAL_MIXED_ZN_TMP = 0.0_EB
MODIFIED_ZETA = 0.0_EB
...
REAL(EB) :: MW_MIXED, RHO_OUT,RHO_MIXED,T1, T2, ZETA0, ZETA_IN_MOD, TMP_IN_MOD, TMP_OUT, CHEM_TIME,&
            DT_MOD, ZETA_ARTF, ZETA_FINAL, ZETA_MAX_LIMIT, ZETA_MIN_LIMIT,FLAME_TMP,EQUIL_TMP
...
IF(IGN_ZN > 0) THEN
   CALL CALC_FLAME_TEMPERATURE(ZZ,TMP_IN,PRES_IN,FLAME_TMP,ZZ_UNMIXED,ZZ_MIXED,ZETA_IN_MOD)
...
IF(ZETA_IN_MOD > ONE_M_EPS) THEN
   ...
   CALL CVODE_SERIAL(CC_CHEM_TIME,ZZ,TMP_IN_MOD,TMP_IN,EQUIL_TMP,USE_DTMPDT_LIMITER,PRES_IN,ZETA0,TAU_MIX,CELL_MASS, &
   ...
   IF (CALL_CHEM_AGAIN) THEN 
      ...
      CVODE_CALL_OPTION = 1 ! CV_NORMAL
      CUR_CFD_TIME = T_CFD ! Set current cfd time in cvode, for logging purpose.
      CALL CVODE_SERIAL(CC,ZZ,TMP_IN_MOD,TMP_IN,EQUIL_TMP,USE_DTMPDT_LIMITER,PRES_IN,ZETA0,TAU_MIX,CELL_MASS,&
...
CUR_CFD_TIME = T_CFD ! Set current cfd time in cvode, for logging purpose.
ZZ_IN = ZZ
CALL  CVODE_SERIAL(CC,ZZ_UNMIXED,TMP_IN_MOD,TMP_IN,EQUIL_TMP,USE_DTMPDT_LIMITER,PRES_IN,ZETA0,TAU_MIX,CELL_MASS,&
                   T1,T2,GLOBAL_ODE_REL_ERROR, ATOL,TMP_OUT, CHEM_TIME, WRITE_SUBSTEPS, CVODE_CALL_OPTION)
...
SUBROUTINE CALC_FLAME_TEMPERATURE(ZZ,TMP_IN,PRES_IN,TMP_OUT,ZZ_UNMIXED,ZZ_MIXED,ZETA)
...
REAL(EB), INTENT(OUT) :: TMP_OUT,ZETA
...
!REAL(EB) :: HS_IN, TMP_AFT, TMP_EQUIL
REAL(EB) :: TMP_EQUIL
...
!TMP_AFT = TMP_IN
!CALL GET_TEMPERATURE(TMP_AFT,HS_IN,ZZ_PROD)

!CALL CALC_EQUIL_TMP_REAC_AND_PROD(ZZ,TMP_IN,PRES_IN,TMP_EQUIL,ZZ_REAC,ZZ_PROD)

CALL CALC_EQUIL_TMP_MIXED_AND_UNMIXED(ZZ,TMP_IN,PRES_IN,TMP_EQUIL,ZZ_MIXED,ZZ_UNMIXED,ZETA)

!IF (TMP_AFT-TMP_EQUIL > 200._EB .OR. (TMP_EQUIL < TMP_OUT)) THEN
!   WRITE(LU_ERR,*)"TMP_AFT,TMP_EQUIL,TMP_AFT-TMP_EQUIL=",TMP_AFT,TMP_EQUIL,TMP_AFT-TMP_EQUIL
!ENDIF

TMP_OUT = TMP_EQUIL

END SUBROUTINE CALC_FLAME_TEMPERATURE


! Calculate Reactants and products
SUBROUTINE CALC_EQUIL_TMP_MIXED_AND_UNMIXED(ZZ,TMP,PRES,TMP_OUT,ZZ_MIXED,ZZ_UNMIXED,ZETA)
...
CALL BUILD_ZZ_MIXED_UNMIXED(ZZ, ZZ_MIXED, ZZ_UNMIXED, ZETA)
...
END SUBROUTINE CALC_EQUIL_TMP_MIXED_AND_UNMIXED


SUBROUTINE SORT_SPECIES(ZZ, SORTED_NS, IS_RICH)
   ...
   DO NS = 1, N_TRACKED_SPECIES
      IF (IS_RICH) THEN
         KEY(NS) = ZZ(NS) * SPECIES_MIXTURE(NS)%OXA
      ELSE
         KEY(NS) = ZZ(NS) * SPECIES_MIXTURE(NS)%OXR
      ENDIF
      SORTED_NS(NS) = NS
   ENDDO

   ! Simple insertion sort (descending)
   DO I = 2, N_TRACKED_SPECIES
      DUMMY = SORTED_NS(I)
      J = I - 1
      DO WHILE (J >= 1)
         IF(KEY(SORTED_NS(J)) < KEY(DUMMY)) THEN
            SORTED_NS(J+1) = SORTED_NS(J)
            J = J - 1
         ELSE
            EXIT
         ENDIF   
...
SUBROUTINE BUILD_ZZ_MIXED_UNMIXED(ZZ, ZZ_MIXED, ZZ_UNMIXED, ZETA)
   USE PHYSICAL_FUNCTIONS, ONLY: CALC_EQUIV_RATIO
   USE CHEMCONS, ONLY: DILUENT_SPECIES_LIST

   REAL(EB), INTENT(IN)  :: ZZ(N_TRACKED_SPECIES)
   REAL(EB), INTENT(OUT) :: ZZ_MIXED(N_TRACKED_SPECIES),ZZ_UNMIXED(N_TRACKED_SPECIES)
   ...
   IF(EQUIV > 1.0_EB) THEN
      IS_RICH = .TRUE.
   ENDIF      
   CALL SORT_SPECIES(ZZ_UNMIXED, SORTED_NS, IS_RICH)
...
END SUBROUTINE BUILD_ZZ_MIXED_UNMIXED


! ! Calculate Reactants and products
! SUBROUTINE CALC_EQUIL_TMP_REAC_AND_PROD(ZZ,TMP,PRES,TMP_OUT,ZZ_REAC,ZZ_PROD)
! USE PHYSICAL_FUNCTIONS, ONLY: CALC_EQUIV_RATIO
! USE CHEMCONS, ONLY: I_FUEL,USE_MIXED_ZN_AFT_TMP,DILUENT_SPECIES_LIST
