...
!$OMP THREADPRIVATE(DZ_F0,RRTMP0,MOLPCM3)
...
ELSE DO_CHEM_LOAD_BALANCE_IF
   ...
   IF (NCHEM_ACTIVE_CELLS_AND_CC >0) THEN
      ...
      MESH_LOOP_SERIAL : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         !$OMP PARALLEL PRIVATE(JCC,I,J,K,CC,ZZ_GET, PRES, DZZ, STOP_STATUS, CHEM_SUBIT_TMP, &
...
INTEGER :: NCHEM_ACTIVE_CELLS, NCHEM_ACTIVE_CC, NC
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES)
...
! SORT NCHEM_ACTIVE_CELLS_AND_CC_RANK_WISE and get the INDEX
SORTED_RANK_INDX = [(I, I = 1, N_MPI_PROCESSES)]
CALL QUICKSORT(NCHEM_ACTIVE_CELLS_AND_CC_RANK_WISE, SORTED_RANK_INDX, 1, N_MPI_PROCESSES, 1)

! Fill the reverse map to point from MY_RANK to SORTED_RANK_INDX
AVG_PIVOT_INDX = -1
AVG = REAL(SUM(NCHEM_ACTIVE_CELLS_AND_CC_RANK_WISE), EB)/REAL(N_MPI_PROCESSES, EB)
REM = MOD(SUM(NCHEM_ACTIVE_CELLS_AND_CC_RANK_WISE), N_MPI_PROCESSES)
...
DO I = 1, N_MPI_PROCESSES
   ...
   SORTED_DIFF_FROM_AVG(I) = NCHEM_ACTIVE_CELLS_AND_CC_RANK_WISE(SORTED_RANK_INDX(I)) - CELLS_PER_SORTED_RANK(I)
   IF (AVG_PIVOT_INDX == -1 .AND. &
      NCHEM_ACTIVE_CELLS_AND_CC_RANK_WISE(SORTED_RANK_INDX(I)) < AVG) THEN
      AVG_PIVOT_INDX = I
   ENDIF
END DO
...
!   DO I = 1, N_MPI_PROCESSES
!      WRITE(LU_ERR,*)"NCHEM_ACTIVE_CELLS_AND_CC, RANK, CELLS_PER_RANK, DIFF=", &
!         NCHEM_ACTIVE_CELLS_AND_CC_RANK_WISE(SORTED_RANK_INDX(I)), &
...
ELSE IF(SORTED_RANK_INDX_MAP(MY_RANK+1) >= AVG_PIVOT_INDX .AND. &
   ...
   NCELLS_TO_RECV_ARRAY_ALGO2(TOT_RANK_TO_COMM,2) = NCHEM_ACTIVE_CELLS_AND_CC_RANK_WISE(MY_RANK+1)
   ...
   ENDIF
   SEQ = 0
   IF (NCHEM_ACTIVE_CELLS_AND_CC_RANK_WISE(MY_RANK+1) > 0) THEN
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         CALL POINT_TO_MESH(NM)
         NCHEM_ACTIVE_CELLS = MESHES(NM)%NCHEM_ACTIVE_CELLS
         NCHEM_ACTIVE_CC = MESHES(NM)%NCHEM_ACTIVE_CC
         ...
         IF (CC_IBM) THEN
            DO NC=1,NCHEM_ACTIVE_CC
               ...
               CHEM_SEQ_ARR(SEQ) = INDX
               REALS_TO_RECV_ARRAY(1:N_TRACKED_SPECIES,INDX) = ZZ_GET
               REALS_TO_RECV_ARRAY(N_TRACKED_SPECIES+1,INDX) = CC%TMP(JCC)
               REALS_TO_RECV_ARRAY(N_TRACKED_SPECIES+2,INDX) = CC%RHO(JCC)
               REALS_TO_RECV_ARRAY(N_TRACKED_SPECIES+3,INDX) = PRES
               REALS_TO_RECV_ARRAY(N_TRACKED_SPECIES+4,INDX) = MU(I,J,K)
               REALS_TO_RECV_ARRAY(N_TRACKED_SPECIES+5,INDX) = LES_FILTER_WIDTH(I,J,K)
               REALS_TO_RECV_ARRAY(N_TRACKED_SPECIES+6,INDX) = CC%VOLUME(JCC)
            ENDDO
         ENDIF ! CC_IBM
      ENDDO ! NM
   ENDIF
