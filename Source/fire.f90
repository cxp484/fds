...
IF (.NOT.ALL(REACTION%FAST_CHEMISTRY)) THEN
   ...
   IF (COMBUSTION_ODE_SOLVER==CVODE_SOLVER .AND. N_IGNITION_ZONES > 0) THEN
      ...
      ENDDO
   ENDIF
   IF (IGN_ZN > 0 .AND. TMP < FINITE_RATE_MIN_TEMP) THEN
         CHEM_ACTIVE = .FALSE.
         RETURN
   ENDIF
ENDIF
...
USE CHEMCONS, ONLY: WRITE_CVODE_SUBSTEPS, MAX_CHEM_TIME ,IGNITION_ZONES
...
IF(ZETA_IN_MOD > ONE_M_EPS) THEN
   ...
   CALL CVODE_SERIAL(CC_CHEM_TIME,ZZ, TMP_IN_MOD, PRES_IN, ZETA0, TAU_MIX, CELL_MASS, T1,T2,  &
   ...
   IF (CHEM_TIME > MAX_CHEM_TIME) THEN ! Limit artifical mixing.
      ...
      CVODE_CALL_OPTION = 1 ! CV_NORMAL
      CUR_CFD_TIME = T_CFD ! Set current cfd time in cvode, for logging purpose.
      CALL CVODE_SERIAL(CC,ZZ, TMP_IN_MOD, PRES_IN, ZETA0, TAU_MIX, CELL_MASS, T1,T2,  &
