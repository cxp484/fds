...
USE CHEMCONS, ONLY: WRITE_CVODE_SUBSTEPS, ZETA_ARTIFICAL_MAX_LIMIT,ZETA_ARTIFICAL_MIN_LIMIT, ZETA_FIRST_STEP_DIV,&
                    USE_MIXED_ZN_AFT_TMP,MIXING_ZN_TMP_LIMIT,MIN_CHEM_TMP,MIN_CHEM_TMP
   ...
   IF (SIM_MODE .NE. DNS_MODE .AND. USE_MIXED_ZN_AFT_TMP) THEN
      ...
      IF (ZETA_IN_MOD > TWO_EPSILON_EB) THEN
         ...
         IF (TMP_UNMIX < 250._EB) THEN
            ...
            DO NS = 1,N_TRACKED_SPECIES
               IF (ZZ(NS) > 0._EB) WRITE(LU_ERR,*) 'Rank, species, Y = ', &
                  MY_RANK,TRIM(SPECIES_MIXTURE(NS)%ID),ZZ(NS)
            ENDDO
         ENDIF
         TMP_UNMIX=MAX(MIN_CHEM_TMP,TMP_UNMIX)
      ELSE
         TMP_UNMIX = TMP_IN
      ENDIF
   ENDIF   
...
USE CHEMCONS, ONLY: I_FUEL,I_CO2,I_CO,I_H2O,I_H2,I_O2,USE_MIXED_ZN_AFT_TMP
REAL(EB), INTENT(INOUT) :: ZZ(N_TRACKED_SPECIES)
REAL(EB), INTENT(IN) :: TMP_IN
REAL(EB), INTENT(OUT) :: AFT
REAL(EB) :: ZZ_EQUIV(N_TRACKED_SPECIES),ZZ_REAC(N_TRACKED_SPECIES),ZZ_PROD(N_TRACKED_SPECIES),ZZ_PROD_BASE(N_TRACKED_SPECIES)
REAL(EB) :: HS_IN,AFT_OLD,EQUIV,X,Y,Z,A,B,C,D,E,F,G,SUM_ZZ,ZZ_FUEL_O2_MASS,BURNED_FUEL_FRACTION,CARBON_ATOMS,&
            H2_EQUIV_MOLES,OXYGEN_ATOMS,H2O_MOLE_OFFSET,H2_MOLE_LIMIT,WGS_KP,CO_MOLES,WGS_QUAD_A,WGS_QUAD_B,&
            WGS_DISCRIMINANT,REAC_MASS,PROD_MASS,FUEL_MASS,O2_MASS,CO2_MASS,H2O_MASS,CO_MASS,H2_MASS
INTEGER :: ITER,NS
REAL(EB), PARAMETER:: AFT_ITER_TOL = 1.E-3_EB
...
DO NS=1, N_TRACKED_SPECIES
   IF (SPECIES_MIXTURE(NS)%OXR > TWENTY_EPSILON_EB .AND. &
      ...
      ZZ_REAC(NS) = 0._EB ! Should only contain N2 + CO2 + H2O  at this point
      ZZ_PROD(NS) = 0._EB ! Should only contain N2 + CO2 + H2O  at this point
...
IF (ZZ_FUEL_O2_MASS > TWENTY_EPSILON_EB) THEN
   ...
   IF (SIM_MODE /= DNS_MODE .AND. USE_MIXED_ZN_AFT_TMP) THEN
      EQUIV = MAX(0.6_EB,EQUIV)
   ENDIF
   CALL GET_ENTHALPY(ZZ,HS_IN,TMP_IN) ! Save the enthalpy.

   ! Lean: CxHyOz + A O2 = B CO2 + C H2O + D CxHyOz + E O2
   ! Rich: CxHyOz + A O2 = B CO2 + C H2O + D CxHyOz + E O2 + F CO + G H2 (Water gas shift reaction)
   ...
   ZZ_REAC = ZZ_REAC/SUM(ZZ_REAC)

   AFT = TMP_IN
   IF (EQUIV < 1.0_EB + EQUIV_TOL) THEN ! Lean and Stoich
   ...
   ELSE ! Rich
      ZZ_PROD_BASE = ZZ_PROD ! Save the base product.

      ! Initial guess for ZZ_PROD (no CO and H2)
      ...
      ! Incorporate water gas shift reaction from Turn Combustion book
      BURNED_FUEL_FRACTION = MIN(1._EB,MAX(0._EB,2._EB*A/MAX(X-Z,TWO_EPSILON_EB)))
      D = 1._EB-BURNED_FUEL_FRACTION ! Rest remain as fuel as sufficient O2 is not available
      CARBON_ATOMS = BURNED_FUEL_FRACTION*X
      H2_EQUIV_MOLES = 0.5_EB*BURNED_FUEL_FRACTION*Y ! Indicator of HYDROGEN_ATOMS (saved in H2 moles for convenience)
      OXYGEN_ATOMS = BURNED_FUEL_FRACTION*Z+2._EB*A
      H2O_MOLE_OFFSET = OXYGEN_ATOMS-2._EB*CARBON_ATOMS
      H2_MOLE_LIMIT = H2_EQUIV_MOLES-H2O_MOLE_OFFSET

      DO ITER=1,10
         WGS_KP = EXP(-2.743_EB+1761._EB/AFT+1.611E6_EB/AFT**2-2.803E8_EB/AFT**3)
         WGS_QUAD_A = WGS_KP-1._EB
         WGS_QUAD_B = WGS_KP*H2O_MOLE_OFFSET+CARBON_ATOMS+H2_MOLE_LIMIT
         WGS_DISCRIMINANT = WGS_QUAD_B**2+4._EB*WGS_QUAD_A*CARBON_ATOMS*H2_MOLE_LIMIT
         CO_MOLES = 2._EB*CARBON_ATOMS*H2_MOLE_LIMIT / &
                  MAX(TWO_EPSILON_EB,WGS_QUAD_B+SQRT(MAX(0._EB,WGS_DISCRIMINANT)))
         CO_MOLES = MIN(MAX(CO_MOLES,MAX(0._EB,-H2O_MOLE_OFFSET)),MIN(CARBON_ATOMS,H2_MOLE_LIMIT))
         B = CARBON_ATOMS-CO_MOLES
         C = H2O_MOLE_OFFSET+CO_MOLES
         F = CO_MOLES
         G = H2_EQUIV_MOLES-C
         CO2_MASS = B*SPECIES_MIXTURE(I_CO2)%MW
         H2O_MASS = C*SPECIES_MIXTURE(I_H2O)%MW
         CO_MASS = F*SPECIES_MIXTURE(I_CO)%MW
         H2_MASS = G*SPECIES_MIXTURE(I_H2)%MW
         FUEL_MASS = D*SPECIES_MIXTURE(I_FUEL)%MW
         PROD_MASS = CO2_MASS+CO_MASS+H2O_MASS+H2_MASS+FUEL_MASS
         ZZ_PROD = ZZ_PROD_BASE
         ZZ_PROD(I_CO2)=ZZ_PROD(I_CO2)+CO2_MASS/PROD_MASS*ZZ_FUEL_O2_MASS
         ZZ_PROD(I_H2O)=ZZ_PROD(I_H2O)+H2O_MASS/PROD_MASS*ZZ_FUEL_O2_MASS
         ZZ_PROD(I_CO)=ZZ_PROD(I_CO)+CO_MASS/PROD_MASS*ZZ_FUEL_O2_MASS
         ZZ_PROD(I_H2)=ZZ_PROD(I_H2)+H2_MASS/PROD_MASS*ZZ_FUEL_O2_MASS
         ZZ_PROD(I_FUEL)=FUEL_MASS/PROD_MASS*ZZ_FUEL_O2_MASS
         ZZ_PROD = ZZ_PROD/SUM(ZZ_PROD)

         AFT_OLD = AFT
         CALL GET_TEMPERATURE(AFT,HS_IN,ZZ_PROD)
         IF (ABS(AFT-AFT_OLD)<=AFT_ITER_TOL*MAX(AFT,1._EB)) EXIT
      ENDDO
