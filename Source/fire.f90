...
AFT = TMP_IN
CALL CALC_EQUIV_RATIO(ZZ(1:N_TRACKED_SPECIES),EQUIV)
IF (EQUIV-1._EB<EQUIV_TOL) THEN
   CALL GET_TEMPERATURE(AFT,HS_IN,ZZ_PROD)
ELSE
   DO ITER=1,10
      CALL GET_TEMPERATURE(AFT,HS_IN,ZZ_PROD)
      CALL CALC_AFT_REAC_AND_PROD(ZZ,ZZ_REAC,ZZ_PROD,AFT)
   ENDDO
   CALL GET_TEMPERATURE(AFT,HS_IN,ZZ_PROD)
ENDIF
...
SUBROUTINE CALC_AFT_REAC_AND_PROD(ZZ,ZZ_REAC,ZZ_PROD,TMP_EQ)
...
REAL(EB), INTENT(IN), OPTIONAL :: TMP_EQ
REAL(EB) :: ZZ_EQUIV(N_TRACKED_SPECIES)
REAL(EB) :: EQUIV,X,Y,Z,A,B,C,D,E,F,G,SUM_ZZ,BURNED_FUEL_FRACTION,CARBON_ATOMS,H2_EQUIV_MOLES,OXYGEN_ATOMS,&
            H2O_MOLE_OFFSET,H2_MOLE_LIMIT,WGS_KP,CO_MOLES,WGS_QUAD_A,WGS_QUAD_B,WGS_DISCRIMINANT
REAL(EB) :: ZZ_MASS, ZZ_FUEL_O2_MASS
REAL(EB) :: REAC_MASS, PROD_MASS, FUEL_MASS, O2_MASS, CO2_MASS, H2O_MASS, CO_MASS,H2_MASS
INTEGER :: NS
...
IF (ZZ_FUEL_O2_MASS > TWENTY_EPSILON_EB) THEN
   ...
   ! Rich: CxHyOz + A O2 = B CO2 + C H2O + D CxHyOz + E O2 + F CO + G H2
   ...
   ELSEIF (EQUIV > 1) THEN ! Rich
      ...
      F = 0._EB
      G = 0._EB
      IF (PRESENT(TMP_EQ) .AND. I_CO>0 .AND. I_H2>0) THEN
         ...
         H2_EQUIV_MOLES = 0.5_EB*BURNED_FUEL_FRACTION*Y
         ...
         WGS_KP = EXP(-2.743_EB+1761._EB/TMP_EQ+1.611E6_EB/TMP_EQ**2-2.803E8_EB/TMP_EQ**3)

         ! Solve the quadratic eqution of Ax^2 + BX +C = 0
   ...
   CO_MASS = 0._EB
   H2_MASS = 0._EB
   IF (I_CO>0) CO_MASS = F*SPECIES_MIXTURE(I_CO)%MW
   IF (I_H2>0) H2_MASS = G*SPECIES_MIXTURE(I_H2)%MW
   FUEL_MASS = D*SPECIES_MIXTURE(I_FUEL)%MW
   O2_MASS = E*SPECIES_MIXTURE(I_O2)%MW
   PROD_MASS = CO2_MASS+CO_MASS+H2O_MASS+H2_MASS+FUEL_MASS+O2_MASS
   ...
   IF (I_CO>0) ZZ_PROD(I_CO)=ZZ_PROD(I_CO)+CO_MASS/PROD_MASS*ZZ_FUEL_O2_MASS
   IF (I_H2>0) ZZ_PROD(I_H2)=ZZ_PROD(I_H2)+H2_MASS/PROD_MASS*ZZ_FUEL_O2_MASS
