...
REAL(EB) :: MIX_TIME_OUT, Q_OUT, CHI_R_OUT, MYTEMP, MYRHO, MYMU, DELTA, VOL, TNOW2,MIX_ZN_TMP
INTEGER :: IGN_ZN
   ...
   IF (NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL > 0) THEN
      ...
      DO INDX=1,NRECEIVE_CELLS
         ...
         CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,&
                                ...
                                MYTEMP,MYRHO,PRES,MYMU,DELTA,VOL,ZETA_0,IGN_ZN,MIX_ZN_TMP)
         !***************************************************************************************
   ...
   IF (COMBUSTION_ODE_SOLVER==CVODE_SOLVER) THEN 
      IF (N_IGNITION_ZONES > 0) THEN
         DO IZ=1,N_IGNITION_ZONES
            ...
            IF (XC(I)>=IGNITION_ZONES(IZ)%X1-TOL .AND. XC(I)<=IGNITION_ZONES(IZ)%X2+TOL .AND. &
                YC(J)>=IGNITION_ZONES(IZ)%Y1-TOL .AND. YC(J)<=IGNITION_ZONES(IZ)%Y2+TOL .AND. &
                ZC(K)>=IGNITION_ZONES(IZ)%Z1-TOL .AND. ZC(K)<=IGNITION_ZONES(IZ)%Z2+TOL) THEN
...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TMP_IN,RHO_IN,PRES_IN,MU_IN,DELTA,CELL_VOLUME,ZETA_0_IN,IGN_ZN,MIX_ZN_TMP_OUT,IIC,JJC,KKC)
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
USE PHYSICAL_FUNCTIONS, ONLY: GET_REALIZABLE_MF,GET_SPECIFIC_HEAT_INTERP
USE COMP_FUNCTIONS, ONLY: SHUTDOWN
USE CHEMCONS, ONLY: ODE_MIN_ATOL,DTMP_MAX,DT_RATIO_FOR_Q_CAP,Q_CAPPED_CELLS,DO_Q_CAPPING
...
REAL(EB) :: A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            ZZ_TEMP(1:N_TRACKED_SPECIES),ATOL(1:N_TRACKED_SPECIES),Q_ALLOWED,CP,DT_MIN_FOR_Q
...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_SUBSTEPS
...
#ifdef WITH_SUNDIALS
         CALL CVODE(ZZ_MIXED,TMP_IN,PRES_IN,ZETA_0, ZETA,TAU_MIX,CELL_MASS,IGN_ZN, &
   ...
   IF (COMBUSTION_ODE_SOLVER == CVODE_SOLVER .AND. DO_Q_CAPPING) THEN
      IF (DT < DT_INITIAL*DT_RATIO_FOR_Q_CAP) THEN
         DT_MIN_FOR_Q = DT_INITIAL*DT_RATIO_FOR_Q_CAP
         CALL GET_SPECIFIC_HEAT_INTERP(ZZ_0(1:N_TRACKED_SPECIES),CP,TMP_IN)
         Q_ALLOWED =  RHO_IN * CP * DTMP_MAX/DT_MIN_FOR_Q
...
#ifdef WITH_SUNDIALS
SUBROUTINE CVODE(ZZ, TMP_IN, PRES_IN,  ZETA_IN, ZETA_OUT, TAU_MIX, CELL_MASS, IGN_ZN, &
...
USE CHEMCONS, ONLY: WRITE_CVODE_SUBSTEPS, ZETA_ARTIFICAL_MAX_LIMIT,ZETA_ARTIFICAL_MIN_LIMIT, ZETA_FIRST_STEP_DIV,&
                    IGNITION_ZONES,USE_MIXED_ZN_AFT_TMP,AFT_ENABLE_TMP                
