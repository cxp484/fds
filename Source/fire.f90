...
REAL(EB) :: MW_MIXED, RHO_OUT,RHO_MIXED,T1, T2, ZETA0, ZETA_IN_MOD, TMP_IN_MOD, TMP_OUT, CHEM_TIME,&
...
ELSE
   IF (SIM_MODE .NE. DNS_MODE) THEN
      IF (TRIM(MIXED_ZN_MODEL) .EQ. "SAME-PHI-KIN-CELLTMP") THEN
         TMP_IN_MOD = TMP_IN
      ELSE IF(TRIM(MIXED_ZN_MODEL) .EQ. "SAME-PHI-KIN-AFT") THEN
      ...
      ELSE IF(TRIM(MIXED_ZN_MODEL) .EQ. "SAME-PHI-KIN-EQUILTMP") THEN
         CALL CALC_EQUIL_FLAME_TEMPERATURE(ZZ,TMP_IN,PRES_IN,FLAME_TMP)
         IF (FLAME_TMP < FINITE_RATE_MIN_TEMP) RETURN
         TMP_IN_MOD = MAX(MIN(FLAME_TMP,MIXING_ZN_TMP_LIMIT),TMP_IN)
         !TMP_IN_MOD = MAX(TMP_IN,FLAME_TMP)
      ELSE IF(TRIM(MIXED_ZN_MODEL) .EQ. "STOICH-PHI-KIN-AFT") THEN
         ...
         TMP_IN_MOD = MAX(MIN(FLAME_TMP,MIXING_ZN_TMP_LIMIT),TMP_IN)
         !TMP_IN_MOD = MAX(TMP_IN,FLAME_TMP)   
      ELSE IF(TRIM(MIXED_ZN_MODEL) .EQ. "STOICH-PHI-KIN-EQUILTMP") THEN
         CALL BUILD_ZZ_UNMIXED_MIXED(ZZ, ZZ_UNMIXED, ZZ_MIXED, ZETA_IN_MOD)
         ZZ_MIXED_DUMMY(1:N_TRACKED_SPECIES)=ZZ_MIXED(1:N_TRACKED_SPECIES)
         ...
         TMP_IN_MOD = MAX(MIN(FLAME_TMP,MIXING_ZN_TMP_LIMIT),TMP_IN)
         !TMP_IN_MOD = MAX(TMP_IN,FLAME_TMP)
      ELSE IF(TRIM(MIXED_ZN_MODEL) .EQ. "SAME-PHI-EQUILIBRATE") THEN
...
RHO_MIXED = PRES_IN*MW_MIXED/R0/TMP_IN 
CC = 0._EB
DO NS =1,N_TRACKED_SPECIES
...
IF(ZETA_IN_MOD > ONE_M_EPS) THEN
   ...
   CALL CVODE_SERIAL(CC_CHEM_TIME,ZZ,ZZ,TMP_IN_MOD,TMP_IN,DUDT,PRES_IN,ZETA0,TAU_MIX,CELL_MASS, &
   ...
   IF (CALL_CHEM_AGAIN) THEN 
      ...
      CVODE_CALL_OPTION = 1 ! CV_NORMAL
      CUR_CFD_TIME = T_CFD ! Set current cfd time in cvode, for logging purpose.
      CALL CVODE_SERIAL(CC,ZZ,ZZ,TMP_IN_MOD,TMP_IN,DUDT,PRES_IN,ZETA0,TAU_MIX,CELL_MASS,&
...
CUR_CFD_TIME = T_CFD ! Set current cfd time in cvode, for logging purpose.
ZZ_IN = ZZ
CALL  CVODE_SERIAL(CC,ZZ_UNMIXED,ZZ,TMP_IN_MOD,TMP_IN,DUDT,PRES_IN,ZETA0,TAU_MIX,CELL_MASS,&
                   T1,T2,GLOBAL_ODE_REL_ERROR, ATOL,TMP_OUT, CHEM_TIME, WRITE_SUBSTEPS, CVODE_CALL_OPTION)
