...
PUBLIC COMBUSTION_LOAD_BALANCED,COMBUSTION_BC,CONDENSATION_EVAPORATION,GET_FLAME_TEMPERATURE, &
       CALC_ADIABATIC_FLAME_TEMPERATURE,CHECK_COMBUSTION_DT
...
USE SOOT_ROUTINES, ONLY: SOOT_SURFACE_OXIDATION
USE COMP_FUNCTIONS, ONLY: CURRENT_TIME
USE CHEMCONS, ONLY: CVODE_WARNING_CELLS,INIT_CVODE_WARN_MESSAGES,Q_CAPPED_CELLS
...
IF (.NOT. COMBUSTION_INIT) THEN
   ...
   NVAR_TO_SEND = N_TRACKED_SPECIES +8 ! NS, TEMP, RHO, PRES, MU, DELTA, VOL, IGN_ZN,SOURCE_CELL_MIX_ZN_TMP; 
   NVAR_TO_RECEIVE = N_TRACKED_SPECIES +5 ! NS, Q_OUT, MIX_TIME_OUT, CHI_R_OUT, CHEM_SUBIT_TMP_OUT, MIX_ZN_TMP
   CALL INIT_CVODE_WARN_MESSAGES()
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      MESHES(NM)%TMP_MIX_OLD=MESHES(NM)%TMP
   ENDDO
ENDIF
...
REAL(EB) :: MIX_TIME_OUT, Q_OUT, CHI_R_OUT, MYTEMP, MYRHO, MYMU, DELTA, VOL, TNOW2,SOURCE_CELL_MIX_ZN_TMP,MIX_ZN_TMP
INTEGER :: IGN_ZN
...
CHEM_LOAD_BALANCE_IF : IF(N_MPI_PROCESSES > 1 .AND. DO_CHEM_LOAD_BALANCE) THEN
   ...
   IF (NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL > 0) THEN
      ...
      CALL DISTRIBUTE_CELLS_ACCROSS_MPI_PROCESSES(NRECEIVE_CELLS,DT)
      ...
      DO INDX=1,NRECEIVE_CELLS
         ...
         SOURCE_CELL_MIX_ZN_TMP= REALS_TO_RECV_ARRAY(N_TRACKED_SPECIES+8,INDX)
         !***************************************************************************************
         ...
         CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,&
                                ...
                                MYTEMP,MYRHO,PRES,MYMU,DELTA,VOL,ZETA_0,IGN_ZN,SOURCE_CELL_MIX_ZN_TMP,MIX_ZN_TMP)
         !***************************************************************************************
         ...
         RESULTS_TO_SEND_ARRAY(N_TRACKED_SPECIES+5,INDX) = MIX_ZN_TMP
   ...
   IF (NCHEM_ACTIVE_CELLS_AND_CC >0) THEN
      ...
      MESH_LOOP_SERIAL : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         DO NC=1,NCHEM_ACTIVE_CELLS
            ...
            CALL GET_SOURCE_CELL_MIX_ZN_TMP(I,J,K,NM,DT,SOURCE_CELL_MIX_ZN_TMP)
            !***************************************************************************************
            ! Call combustion integration routine for Cartesian cell (I,J,K)
            CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q(I,J,K),MIX_TIME(I,J,K),CHI_R(I,J,K),&
                                   ...
                                   LES_FILTER_WIDTH(I,J,K),DX(I)*DY(J)*DZ(K),ZETA_0,IGN_ZN,SOURCE_CELL_MIX_ZN_TMP,&
                                   TMP_MIX(I,J,K),IIC=I,JJC=J,KKC=K)
            !***************************************************************************************
            !IF (STOP_STATUS/=NO_STOP) RETURN
         ...
         IF (CC_IBM) THEN
            ...
            DO NC=1,NCHEM_ACTIVE_CC
               ...
               CALL COMBUSTION_MODEL( T,DT,ZZ_GET,CC%Q(JCC),CC%MIX_TIME(JCC),&
                                      ...
                                      LES_FILTER_WIDTH(I,J,K),CC%VOLUME(JCC),ZETA_0,IGN_ZN,SOURCE_CELL_MIX_ZN_TMP,&
                                      TMP_MIX(I,J,K),IIC=I,JJC=J,KKC=K)
               !***************************************************************************************
...
USE CHEMCONS, ONLY: EQUIV_RATIO_CHECK,MIN_EQUIV_RATIO,MAX_EQUIV_RATIO,N_IGNITION_ZONES,IGNITION_ZONES,MIXED_ZN_TMP_MODEL, &
                    CELL_TMP_MODEL
...
IF (.NOT.ALL(REACTION%FAST_CHEMISTRY)) THEN
   ...
   IF (IGN_ZN .LE. 0 .AND. SIM_MODE .NE. DNS_MODE .AND. MIXED_ZN_TMP_MODEL .NE. CELL_TMP_MODEL ) THEN ! LES and other modes. Check equiv ratio
      CALL CALC_EQUIV_RATIO(ZZ_GET(1:N_TRACKED_SPECIES), EQUIV)
      ! No need to solve for no fuel (phi < MIN ) case and only fuel (phi is rich > MAX) case.
      IF (EQUIV .GT. MIN_EQUIV_RATIO .AND. EQUIV .LT. MAX_EQUIV_RATIO) THEN
...
SUBROUTINE GET_SOURCE_CELL_MIX_ZN_TMP(I,J,K,NM,DT,TMP_NEIGH)
USE TRAN, ONLY : GET_IJK
INTEGER, INTENT(IN) :: I,J,K,NM
REAL(EB), INTENT(IN) :: DT
REAL(EB), INTENT(OUT) :: TMP_NEIGH

INTEGER:: I_NEIGH,J_NEIGH,K_NEIGH
REAL(EB) :: X_NEIGH,Y_NEIGH,Z_NEIGH,XI,YJ,ZK

X_NEIGH=MESHES(NM)%XC(I) - 0.5_EB*(MESHES(NM)%U(I,J,K)+MESHES(NM)%U(I-1,J,K))*DT
Y_NEIGH=MESHES(NM)%YC(J) - 0.5_EB*(MESHES(NM)%V(I,J,K)+MESHES(NM)%V(I,J-1,K))*DT
Z_NEIGH=MESHES(NM)%ZC(K) - 0.5_EB*(MESHES(NM)%W(I,J,K)+MESHES(NM)%W(I,J,K-1))*DT

CALL GET_IJK(X_NEIGH,Y_NEIGH,Z_NEIGH,NM,XI,YJ,ZK,I_NEIGH,J_NEIGH,K_NEIGH)
I_NEIGH = MAX(-1,I_NEIGH)
J_NEIGH = MAX(-1,J_NEIGH)
K_NEIGH = MAX(-1,K_NEIGH)
TMP_NEIGH = MESHES(NM)%TMP_MIX_OLD(I_NEIGH,J_NEIGH,K_NEIGH)

END SUBROUTINE GET_SOURCE_CELL_MIX_ZN_TMP


SUBROUTINE DISTRIBUTE_CELLS_ACCROSS_MPI_PROCESSES (NRECEIVE_CELLS,DT)
...
REAL(EB) :: PRES, TNOW, SRC_CELL_MIXZN_TMP
TYPE (CC_CUTCELL_TYPE), POINTER :: CC
...
IF (NCHEM_ACTIVE_CELLS_AND_CC > 0) THEN
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      DO NC=1,NCHEM_ACTIVE_CELLS
         ...
         ! Reals to send:
         ZZ_GET = ZZ(I,J,K,1:N_TRACKED_SPECIES)
         CALL GET_SOURCE_CELL_MIX_ZN_TMP(I,J,K,NM,DT,SRC_CELL_MIXZN_TMP)
         PRES = PBAR(K,PRESSURE_ZONE(I,J,K)) + RHO(I,J,K)*(H(I,J,K)-KRES(I,J,K))
         ...
         REALS_TO_SEND_ARRAY(N_TRACKED_SPECIES+8,INDX) = SRC_CELL_MIXZN_TMP 
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO NC=1,NCHEM_ACTIVE_CELLS
      ...
      IF (OUTPUT_CHEM_IT) CHEM_SUBIT(I,J,K) = CHEM_SUBIT_TMP
      TMP_MIX(I,J,K)       = RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+5,INDX)
      CALL SET_SPECIES_SOURCE_TERM_CELL(DT, I, J, K, ZZ_GET, DZZ, REAC_SOURCE_TERM_TMP, Q_REAC_TMP)
      IF (STOP_STATUS/=NO_STOP) RETURN
   ENDDO
   ...
   IF (CC_IBM) THEN
      DO NC=1,NCHEM_ACTIVE_CC
         ...
         CHEM_SUBIT_TMP       = INT(RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+4,INDX))
         !CC%TMP_MIX(JCC)      = RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+5,INDX)
         CALL SET_SPECIES_SOURCE_TERM_CUTCELL(DT, ICC, JCC, ZZ_GET, DZZ, REAC_SOURCE_TERM_TMP, Q_REAC_TMP)
      ENDDO
   ENDIF ! CC_IBM
ENDDO  ! NM
...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TMP_IN,RHO_IN,PRES_IN,MU_IN,DELTA,CELL_VOLUME,ZETA_0_IN,IGN_ZN,SOURCE_CELL_MIX_ZN_TMP_IN, &
                            MIX_ZN_TMP_OUT,IIC,JJC,KKC)
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
USE PHYSICAL_FUNCTIONS, ONLY: GET_REALIZABLE_MF,GET_SPECIFIC_HEAT_INTERP
USE COMP_FUNCTIONS, ONLY: SHUTDOWN
USE CHEMCONS, ONLY: ODE_MIN_ATOL,DTMP_MAX,DT_MIN_FOR_Q,Q_CAPPED_CELLS
INTEGER, INTENT(IN) :: IGN_ZN
INTEGER, INTENT(IN), OPTIONAL :: IIC,JJC,KKC
REAL(EB), INTENT(IN) :: T,DT,RHO_IN,PRES_IN,MU_IN,DELTA,CELL_VOLUME,ZETA_0_IN,SOURCE_CELL_MIX_ZN_TMP_IN
REAL(EB), INTENT(OUT) :: Q_OUT,MIX_TIME_OUT,CHI_R_OUT,MIX_ZN_TMP_OUT,&
                        REAC_SOURCE_TERM_OUT(N_TRACKED_SPECIES),Q_REAC_OUT(N_REACTIONS)
...
REAL(EB) :: A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            ZZ_TEMP(1:N_TRACKED_SPECIES),ATOL(1:N_TRACKED_SPECIES),Q_ALLOWED,CP
...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_SUBSTEPS
...
#ifdef WITH_SUNDIALS
         CALL CVODE(ZZ_MIXED,TMP_IN,PRES_IN,ZETA_0, ZETA,TAU_MIX,CELL_MASS,IGN_ZN,SOURCE_CELL_MIX_ZN_TMP_IN,&
                  MIX_ZN_TMP_OUT,T,DT_SUB,GLOBAL_ODE_REL_ERROR, ATOL)
#endif
         ZETA_0     = ZETA
         Q_REAC_SUB = 0._EB
   ...
   ! Modify Zeta to limit Q, in case it is very high to make numerical solver unstable
   IF (COMBUSTION_ODE_SOLVER == CVODE_SOLVER) THEN
      CALL GET_SPECIFIC_HEAT_INTERP(ZZ_0(1:N_TRACKED_SPECIES),CP,TMP_IN)
      Q_ALLOWED =  RHO_IN * CP * DTMP_MAX/(MAX(DT,DT_MIN_FOR_Q))
      Q_OUT = -RHO_IN*(1-ZETA)*SUM(SPECIES_MIXTURE%H_F*(ZZ_MIXED-ZZ_0))/DT ! Another form of FDS Tech Guide (5.47)
      IF (ABS(Q_OUT) > ABS(Q_ALLOWED)) THEN
...
#ifdef WITH_SUNDIALS
SUBROUTINE CVODE(ZZ, TMP_IN, PRES_IN,  ZETA_IN, ZETA_OUT, TAU_MIX, CELL_MASS, IGN_ZN, SOURCE_CELL_MIX_ZN_TMP_IN, &
                 MIX_ZN_TMP_OUT,T_CFD, DT, GLOBAL_ODE_REL_ERROR, ATOL)
...
USE CHEMCONS, ONLY: WRITE_CVODE_SUBSTEPS, ZETA_ARTIFICAL_MAX_LIMIT,ZETA_ARTIFICAL_MIN_LIMIT, ZETA_FIRST_STEP_DIV,&
                    IGNITION_ZONES,MIXED_ZN_TMP_MODEL,CELL_TMP_MODEL,AFT_EVERYWHERE_MODEL, &
                    SOURCE_MIXZN_TMP_MODEL                  
...
REAL(EB), INTENT(IN) :: TMP_IN,PRES_IN,ZETA_IN,TAU_MIX,CELL_MASS,T_CFD,DT,GLOBAL_ODE_REL_ERROR,&
                        SOURCE_CELL_MIX_ZN_TMP_IN
REAL(EB), INTENT(OUT) ::ZETA_OUT,MIX_ZN_TMP_OUT
INTEGER, INTENT(IN) :: IGN_ZN
...
LOGICAL :: WRITE_SUBSTEPS, CALL_CHEM_AGAIN,CAP_LEAN
...
ELSE
   IF (SIM_MODE .NE. DNS_MODE .AND. MIXED_ZN_TMP_MODEL .NE. CELL_TMP_MODEL) THEN
      IF (MIXED_ZN_TMP_MODEL .EQ. AFT_EVERYWHERE_MODEL) THEN
         CAP_LEAN = .TRUE.
         CALL CALC_ADIABATIC_FLAME_TEMPERATURE(ZZ,TMP_IN,CAP_LEAN,AFT)
         TMP_IN_MOD = MAX(TMP_IN,AFT) ! TO DO: Ideal would be a equilibrium temperature based on ZZ(:) and TMP_IN
         IF (TMP_IN_MOD < FINITE_RATE_MIN_TEMP) RETURN
      ELSEIF (MIXED_ZN_TMP_MODEL .EQ. SOURCE_MIXZN_TMP_MODEL) THEN 
         IF (SOURCE_CELL_MIX_ZN_TMP_IN > FINITE_RATE_MIN_TEMP) THEN
            CAP_LEAN = .TRUE.
            CALL CALC_ADIABATIC_FLAME_TEMPERATURE(ZZ,TMP_IN,CAP_LEAN,AFT)
            TMP_IN_MOD = MAX(AFT, TMP_IN)
         ENDIF
         IF (TMP_IN_MOD < FINITE_RATE_MIN_TEMP) RETURN
      ENDIF      
   ENDIF   
...
! Calculate RHO based on actual temperature, such that RHO and Concentration comes out to be same as in the cell.
...
MIX_ZN_TMP_OUT = MIN(REAL(I_MAX_TEMP,EB),MAX(0._EB,TMP_OUT))
...
SUBROUTINE CALC_ADIABATIC_FLAME_TEMPERATURE(ZZ,TMP_IN,CAP_LEAN,AFT)
...
IF (I_FUEL <= 0) THEN ! No FUEL_ID_FOR_AFT specified. 
   ...
   RETURN
ENDIF
CALL CALC_AFT_REAC_AND_PROD(ZZ,CAP_LEAN,ZZ_REAC,ZZ_PROD)
...
SUBROUTINE CALC_AFT_REAC_AND_PROD(ZZ,CAP_LEAN,ZZ_REAC,ZZ_PROD)
...
IF (ZZ_FUEL_O2_MASS > TWO_EPSILON_EB) THEN
   ...
   A=0.5_EB*((2._EB*X+0.5_EB*Y)/(EQUIV+TWO_EPSILON_EB) -Z) ! a is a function of equivalence ratio
...
SUBROUTINE DUMP_CVODE_WARNING_SUMMARY(NCHEM_ACTIVE_CELLS_AND_CC)
   USE CHEMCONS, ONLY: CVODE_WARNING_CELLS,CVODE_ERR_CODE_MIN,CVODE_ERR_CODE_MAX,CVODE_WARN_MESSAGES,Q_CAPPED_CELLS

   INTEGER, INTENT(IN) :: NCHEM_ACTIVE_CELLS_AND_CC
   INTEGER :: NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL,IERR, CVODE_ERR_CODE, TOT_WARN_CELLS,Q_CAPPED_CELLS_GLOBAL
   REAL(EB) :: PCNT,TNOW
   ...
   IF (N_MPI_PROCESSES > 1) THEN
      ...
      CALL MPI_ALLREDUCE(Q_CAPPED_CELLS, Q_CAPPED_CELLS_GLOBAL, &
                         1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, IERR)
      
   ELSE
      NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL = NCHEM_ACTIVE_CELLS_AND_CC
      Q_CAPPED_CELLS_GLOBAL = Q_CAPPED_CELLS
   END IF
   TOT_WARN_CELLS = SUM(CVODE_WARNING_CELLS)

   IF ((TOT_WARN_CELLS > 0 .OR. Q_CAPPED_CELLS_GLOBAL > 0) .AND. &
         NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL > 0 .AND. MY_RANK == 0) THEN
      DASHED_LINE = .TRUE.
   ...
   IF (TOT_WARN_CELLS > 0 .AND. NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL > 0 .AND. MY_RANK == 0) THEN
      ...
      DO CVODE_ERR_CODE = CVODE_ERR_CODE_MIN, CVODE_ERR_CODE_MAX
         IF (CVODE_WARNING_CELLS(CVODE_ERR_CODE) > 0) THEN
            ...
            PCNT = REAL(CVODE_WARNING_CELLS(CVODE_ERR_CODE),EB) / &
                   REAL(NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL,EB) * 100.0_EB
   ...
   IF (Q_CAPPED_CELLS_GLOBAL > 0 .AND. NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL > 0 .AND. MY_RANK == 0) THEN
      ! Write Q_capped cells
      PCNT = REAL(Q_CAPPED_CELLS_GLOBAL,EB) / &
                   REAL(NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL,EB) * 100.0_EB
      WRITE(CELL_STR, '(I0)') Q_CAPPED_CELLS_GLOBAL
      WRITE(PCNT_STR, '(F6.2)') PCNT
      CELL_STR = ADJUSTL(CELL_STR)
      PCNT_STR = ADJUSTL(PCNT_STR)
      ...
      ! Print one line of summary
      WRITE(LU_ERR, '(1X, A, 5X, A)') "Qdot capped cells = ", TRIM(CELL_INFO)
   ENDIF   
...
SUBROUTINE CHECK_COMBUSTION_DT(DT_NEW)
   USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_HEAT_INTERP
   USE CHEMCONS, ONLY : DTMP_MAX

   REAL(EB), INTENT(INOUT) :: DT_NEW(NMESHES)
   REAL(EB) :: CP,ZZ_GET(N_TRACKED_SPECIES),DT_CALC,MIN_DT,Q_ABS,VCELL, MAX_Q
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               Q_ABS = ABS(Q(I,J,K))
               MAX_Q=MAX(MAX_Q,Q_ABS)
               ZZ_GET = ZZ(I,J,K,1:N_TRACKED_SPECIES)
               CALL GET_SPECIFIC_HEAT_INTERP(ZZ_GET(1:N_TRACKED_SPECIES),CP,TMP(I,J,K))
               DT_CALC= RHO(I,J,K)*CP*DTMP_MAX/(Q_ABS+TWO_EPSILON_EB)
               MIN_DT= MIN(MIN_DT,DT_CALC)
            ENDDO
         ENDDO
      ENDDO
      ...
      IF (CC_IBM) THEN
         DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
            ...
            DO JCC=1,CC%NCELL
               ! Drop if cut-cell is very small compared to Cartesian cells:
               IF ( ABS(CUT_CELL(ICC)%VOLUME(JCC)/VCELL) <  1.E-12_EB ) CYCLE
               Q_ABS = ABS(CUT_CELL(ICC)%Q(JCC))
               MAX_Q=MAX(MAX_Q,Q_ABS)
               ZZ_GET = CUT_CELL(ICC)%ZZ(1:N_TRACKED_SPECIES,JCC)
               CALL GET_SPECIFIC_HEAT_INTERP(ZZ_GET(1:N_TRACKED_SPECIES),CP,CUT_CELL(ICC)%TMP(JCC))
               DT_CALC= RHO(I,J,K)*CP*DTMP_MAX/(Q_ABS+TWO_EPSILON_EB)
               MIN_DT= MIN(MIN_DT,DT_CALC)
            ENDDO
         ENDDO
      ENDIF   
      WRITE(LU_ERR,*)"My RANK,NM=:",MY_RANK,NM," VEL DT=",DT_NEW(NM)," COMB DT=",MIN_DT, "MAX_Q=",MAX_Q
      DT_NEW(NM) = MIN(DT_NEW(NM),MIN_DT)      
   ENDDO 
END SUBROUTINE CHECK_COMBUSTION_DT
