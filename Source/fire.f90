...
USE SOOT_ROUTINES, ONLY: SOOT_SURFACE_OXIDATION
USE COMP_FUNCTIONS, ONLY: CURRENT_TIME
USE CHEMCONS, ONLY: CVODE_WARNING_CELLS,INIT_CVODE_WARN_MESSAGES,EQUIL_WARNING_CELLS
...
IF (.NOT. COMBUSTION_INIT) THEN
   ...
   NVAR_TO_RECEIVE = N_TRACKED_SPECIES +7 ! NS, Q_OUT, MIX_TIME_OUT, CHI_R_OUT, CHEM_SUBIT_TMP_OUT, INITIAL_MIXED_ZN_TMP, FINAL_MIXED_ZN_TMP, MOD_ZETA
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   INITIAL_TMP_MIX = TMP
   FINAL_TMP_MIX = TMP
   MOD_ZETA = INITIAL_UNMIXED_FRACTION
   CVODE_WARNING_CELLS = 0
   EQUIL_WARNING_CELLS = 0
   IF (CC_IBM) THEN
...
REAl(EB) :: INITIAL_MIXED_ZN_TMP, FINAL_MIXED_ZN_TMP, MODIFIED_ZETA
INTEGER :: IGN_ZN
   ...
   IF (NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL > 0) THEN
      ...
      DO INDX=1,NRECEIVE_CELLS
         ...
         CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,&
                                ...
                                INITIAL_MIXED_ZN_TMP,FINAL_MIXED_ZN_TMP,MODIFIED_ZETA)
         !***************************************************************************************
         ...
         RESULTS_TO_SEND_ARRAY(N_TRACKED_SPECIES+5,INDX) = INITIAL_MIXED_ZN_TMP
         RESULTS_TO_SEND_ARRAY(N_TRACKED_SPECIES+6,INDX) = FINAL_MIXED_ZN_TMP
         RESULTS_TO_SEND_ARRAY(N_TRACKED_SPECIES+7,INDX) = MODIFIED_ZETA
      ENDDO
      ...
      MESH_LOOP_SERIAL : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         DO NC=1,NCHEM_ACTIVE_CELLS
            ...
            CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q(I,J,K),MIX_TIME(I,J,K),CHI_R(I,J,K),&
                                   ...
                                   LES_FILTER_WIDTH(I,J,K),DX(I)*DY(J)*DZ(K),ZETA_0,IGN_ZN,&
                                   INITIAL_TMP_MIX(I,J,K),FINAL_TMP_MIX(I,J,K),MOD_ZETA(I,J,K),IIC=I,JJC=J,KKC=K )
            !***************************************************************************************
            !IF (STOP_STATUS/=NO_STOP) RETURN
         ...
         IF (CC_IBM) THEN
            ...
            DO NC=1,NCHEM_ACTIVE_CC
               ...
               CALL COMBUSTION_MODEL( T,DT,ZZ_GET,CC%Q(JCC),CC%MIX_TIME(JCC),&
                                      ...
                                      LES_FILTER_WIDTH(I,J,K),CC%VOLUME(JCC),ZETA_0,IGN_ZN, &
                                      INITIAL_TMP_MIX(I,J,K),FINAL_TMP_MIX(I,J,K),MOD_ZETA(I,J,K),IIC=I,JJC=J,KKC=K)
               !***************************************************************************************
...
IF (.NOT.ALL(REACTION%FAST_CHEMISTRY)) THEN
   ...
   IF (IGN_ZN .LE. 0 .AND. SIM_MODE .NE. DNS_MODE .AND. TRIM(MIXED_ZN_MODEL) .NE. "SAME-PHI-KIN-CELLTMP") THEN ! LES and other modes. Check equiv ratio
      CALL CALC_EQUIV_RATIO(ZZ_GET(1:N_TRACKED_SPECIES), EQUIV)
      ! No need to solve for no fuel (phi < MIN ) case and only fuel (phi is rich > MAX) case.
      IF (EQUIV .GT. MIN_EQUIV_RATIO .AND. EQUIV .LT. MAX_EQUIV_RATIO) THEN
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO NC=1,NCHEM_ACTIVE_CELLS
      ...
      IF (OUTPUT_CHEM_IT) CHEM_SUBIT(I,J,K) = CHEM_SUBIT_TMP
      INITIAL_TMP_MIX(I,J,K)      = RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+5,INDX)
      FINAL_TMP_MIX(I,J,K)        = RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+6,INDX)
      MOD_ZETA(I,J,K)             = RESULTS_TO_RECV_ARRAY(N_TRACKED_SPECIES+7,INDX)
      CALL SET_SPECIES_SOURCE_TERM_CELL(DT, I, J, K, ZZ_GET, DZZ, REAC_SOURCE_TERM_TMP, Q_REAC_TMP)
      IF (STOP_STATUS/=NO_STOP) RETURN
   ENDDO
...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TMP_IN,RHO_IN,PRES_IN,MU_IN,DELTA,CELL_VOLUME,ZETA_0_IN,IGN_ZN, &
                            INITIAL_MIXED_ZN_TMP,FINAL_MIXED_ZN_TMP,MODIFIED_ZETA,IIC,JJC,KKC)
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
...
REAl(EB), INTENt(OUT) :: INITIAL_MIXED_ZN_TMP,FINAL_MIXED_ZN_TMP,MODIFIED_ZETA
...
REAL(EB) :: A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            ZZ_TEMP(1:N_TRACKED_SPECIES),ATOL(1:N_TRACKED_SPECIES),ZZ_UNMIXED(1:N_TRACKED_SPECIES)
...
INITIAL_MIXED_ZN_TMP = TMP_IN
FINAL_MIXED_ZN_TMP = TMP_IN
MODIFIED_ZETA = ZETA_0_IN
...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_SUBSTEPS
...
#ifdef WITH_SUNDIALS
         ZZ_UNMIXED = 0.0_EB
         CALL CVODE(ZZ_MIXED,ZZ_UNMIXED,TMP_IN,PRES_IN,ZETA_0, ZETA,TAU_MIX,CELL_MASS,IGN_ZN,T,DT_SUB,&
               GLOBAL_ODE_REL_ERROR, ATOL, INITIAL_MIXED_ZN_TMP,FINAL_MIXED_ZN_TMP,MODIFIED_ZETA)
#endif
         ZETA_0     = ZETA
         Q_REAC_SUB = 0._EB
   ...
   IF (COMBUSTION_ODE_SOLVER == CVODE_SOLVER ) THEN
      ZZ_GET = ZETA*ZZ_UNMIXED + (1._EB-ZETA)*ZZ_MIXED 
   ELSE
...
!> \param ZZ_UNMIXED species mass fraction of unmixed zone (OUT) 
...
#ifdef WITH_SUNDIALS
SUBROUTINE CVODE(ZZ, ZZ_UNMIXED,TMP_IN, PRES_IN,  ZETA_IN, ZETA_OUT, TAU_MIX, CELL_MASS, &
      IGN_ZN, T_CFD, DT, GLOBAL_ODE_REL_ERROR, ATOL,INITIAL_MIXED_ZN_TMP,FINAL_MIXED_ZN_TMP,MODIFIED_ZETA)
...
USE CHEMCONS, ONLY: WRITE_CVODE_SUBSTEPS, ZETA_ARTIFICAL_MAX_LIMIT,ZETA_ARTIFICAL_MIN_LIMIT, ZETA_FIRST_STEP_DIV,&
                    MIXED_ZN_MODEL,IGNITION_ZONES,ADJUST_ENERGY_FOR_MIXING_ZN,MIXING_ZN_TMP_LIMIT
REAL(EB), INTENT(INOUT) :: ZZ(N_TRACKED_SPECIES)
REAL(EB), INTENT(OUT) :: ZZ_UNMIXED(N_TRACKED_SPECIES)
...
REAL(EB), INTENT(OUT) ::ZETA_OUT,INITIAL_MIXED_ZN_TMP,FINAL_MIXED_ZN_TMP,MODIFIED_ZETA
INTEGER, INTENT(IN) :: IGN_ZN

REAL(EB) :: ZZ_MIXED(1:N_TRACKED_SPECIES),ZZ_MIXED_DUMMY(1:N_TRACKED_SPECIES)
REAL(EB) :: CC(N_TRACKED_SPECIES), CC_CHEM_TIME(N_TRACKED_SPECIES), ZZ_IN(N_TRACKED_SPECIES)
REAL(EB) :: RHO_IN,RHO_OUT,RHO_MIXED,T1, T2, ZETA0, ZETA_IN_MOD, TMP_IN_MOD, TMP_OUT, CHEM_TIME,&
            DT_MOD, ZETA_ARTF, ZETA_FINAL, ZETA_MAX_LIMIT, ZETA_MIN_LIMIT,FLAME_TMP,PRES_OUT
REAL(EB) :: DUDT,PRES_MIXED,MW,MW_UNMIXED,MW_MIXED
INTEGER :: NS, CVODE_CALL_OPTION
LOGICAL :: WRITE_SUBSTEPS, CALL_CHEM_AGAIN, USE_DTMPDT_LIMITER
...
DUDT = 0.0_EB
USE_DTMPDT_LIMITER = .FALSE.

ZZ_MIXED(1:N_TRACKED_SPECIES)=ZZ(1:N_TRACKED_SPECIES)
ZZ_UNMIXED(1:N_TRACKED_SPECIES)=ZZ(1:N_TRACKED_SPECIES)

IF(IGN_ZN > 0) THEN
   TMP_IN_MOD = MAX(TMP_IN, IGNITION_ZONES(IGN_ZN)%TMP)
   ZETA_IN_MOD = MIN(MAX(0._EB, IGNITION_ZONES(IGN_ZN)%ZETA),1._EB)
ELSE
   IF (SIM_MODE .NE. DNS_MODE) THEN
      IF (TRIM(MIXED_ZN_MODEL) .EQ. "SAME-PHI-KIN-CELLTMP") THEN
         TMP_IN_MOD = TMP_IN
      ELSEIF(TRIM(MIXED_ZN_MODEL) .EQ. "SAME-PHI-KIN-AFT") THEN
         CALL CALC_ADIABATIC_FLAME_TEMPERATURE(ZZ,TMP_IN,FLAME_TMP)
         IF (FLAME_TMP < FINITE_RATE_MIN_TEMP) RETURN
         TMP_IN_MOD = MAX(MIN(FLAME_TMP,MIXING_ZN_TMP_LIMIT),TMP_IN)
         !TMP_IN_MOD = MAX(TMP_IN,FLAME_TMP)
      ELSEIF(TRIM(MIXED_ZN_MODEL) .EQ. "SAME-PHI-KIN-EQUILTMP") THEN
         CALL CALC_EQUIL_FLAME_TEMPERATURE(ZZ,TMP_IN,PRES_IN,FLAME_TMP)
         IF (FLAME_TMP < FINITE_RATE_MIN_TEMP) RETURN
         TMP_IN_MOD = MAX(MIN(FLAME_TMP,MIXING_ZN_TMP_LIMIT),TMP_IN)
         !TMP_IN_MOD = MAX(TMP_IN,FLAME_TMP)
      ELSEIF(TRIM(MIXED_ZN_MODEL) .EQ. "STOICH-PHI-KIN-AFT") THEN
         CALL BUILD_ZZ_UNMIXED_MIXED(ZZ, ZZ_UNMIXED, ZZ_MIXED, ZETA_IN_MOD)
         ZZ_MIXED_DUMMY(1:N_TRACKED_SPECIES)=ZZ_MIXED(1:N_TRACKED_SPECIES)
         CALL CALC_ADIABATIC_FLAME_TEMPERATURE(ZZ_MIXED_DUMMY,TMP_IN,FLAME_TMP)
         IF (FLAME_TMP < FINITE_RATE_MIN_TEMP) RETURN
         TMP_IN_MOD = MAX(MIN(FLAME_TMP,MIXING_ZN_TMP_LIMIT),TMP_IN)
         !TMP_IN_MOD = MAX(TMP_IN,FLAME_TMP)   
      ELSEIF(TRIM(MIXED_ZN_MODEL) .EQ. "STOICH-PHI-KIN-EQUILTMP") THEN
         CALL BUILD_ZZ_UNMIXED_MIXED(ZZ, ZZ_UNMIXED, ZZ_MIXED, ZETA_IN_MOD)
         ZZ_MIXED_DUMMY(1:N_TRACKED_SPECIES)=ZZ_MIXED(1:N_TRACKED_SPECIES)
         CALL EQUILIBRATE(ZZ_MIXED_DUMMY,TMP_IN,PRES_IN,FLAME_TMP,PRES_OUT,1)
         IF (FLAME_TMP < FINITE_RATE_MIN_TEMP) RETURN
         TMP_IN_MOD = MAX(MIN(FLAME_TMP,MIXING_ZN_TMP_LIMIT),TMP_IN)
         !TMP_IN_MOD = MAX(TMP_IN,FLAME_TMP)
      ELSEIF(TRIM(MIXED_ZN_MODEL) .EQ. "SAME-PHI-EQUILIBRATE") THEN
         ! Just equilibrate the mixed zone, no need to solve kinetics.
         CALL EQUILIBRATE(ZZ_MIXED,TMP_IN,PRES_IN,FLAME_TMP,PRES_OUT,1)
         ZETA_OUT = ZETA_IN_MOD*EXP(-DT/TAU_MIX)
         ZZ = ZZ_MIXED
         RETURN
      ENDIF

      IF (ADJUST_ENERGY_FOR_MIXING_ZN .AND. TRIM(MIXED_ZN_MODEL) .NE. "SAME-PHI-KIN-CELLTMP") THEN
         CALL CALC_DUDT_LOSS(ZZ_UNMIXED,TMP_IN,ZZ_MIXED,TMP_IN_MOD,ZETA_IN_MOD,CELL_MASS,DT,DUDT)
      ENDIF
   ENDIF   
ENDIF
ZETA_OUT = ZETA_IN_MOD*EXP(-DT/TAU_MIX)
...
CALL GET_MOLECULAR_WEIGHT(ZZ_UNMIXED,MW_UNMIXED)
RHO_IN = PRES_IN*MW/R0/TMP_IN ! [PR]= Pa, [MW] = g/mol, [R0]= J/K/kmol, [TMP]=K, [RHO]= kg/m3
PRES_MIXED = RHO_IN*R0*(ZETA_IN_MOD*TMP_IN/MW_UNMIXED+(1.0_EB - ZETA_IN_MOD)*TMP_IN_MOD/MW_MIXED) ! To conserve volume.
!WRITE(LU_ERR,*)"PRES_IN,PRES_MIXED=",PRES_IN,PRES_MIXED

RHO_MIXED = PRES_MIXED*MW_MIXED/R0/TMP_IN_MOD 
CC = 0._EB
DO NS =1,N_TRACKED_SPECIES
  CC(NS) = RHO_MIXED*ZZ_MIXED(NS)/SPECIES_MIXTURE(NS)%MW  ! [RHO]= kg/m3, [MW] = g/mol = kg/kmol, [CC] = kmol/m3
ENDDO
...
IF(ZETA_IN_MOD > ONE_M_EPS) THEN
   ...
   CALL CVODE_SERIAL(CC_CHEM_TIME,ZZ,TMP_IN_MOD,TMP_IN,DUDT,PRES_MIXED,ZETA0,TAU_MIX,CELL_MASS, &
                      T1,T2,GLOBAL_ODE_REL_ERROR, ATOL, TMP_OUT, CHEM_TIME, WRITE_SUBSTEPS, CVODE_CALL_OPTION) ! Find the chem time scale
   ...
   IF (CALL_CHEM_AGAIN) THEN 
      ...
      CVODE_CALL_OPTION = 1 ! CV_NORMAL
      CUR_CFD_TIME = T_CFD ! Set current cfd time in cvode, for logging purpose.
      CALL CVODE_SERIAL(CC,ZZ,TMP_IN_MOD,TMP_IN,DUDT,PRES_MIXED,ZETA0,TAU_MIX,CELL_MASS,&
                        T1,T2,GLOBAL_ODE_REL_ERROR, ATOL, TMP_OUT, CHEM_TIME, WRITE_SUBSTEPS, CVODE_CALL_OPTION)
      CHEM_TIME = T2
   ELSE
...
CUR_CFD_TIME = T_CFD ! Set current cfd time in cvode, for logging purpose.
ZZ_IN = ZZ
CALL  CVODE_SERIAL(CC,ZZ_UNMIXED,TMP_IN_MOD,TMP_IN,DUDT,PRES_MIXED,ZETA0,TAU_MIX,CELL_MASS,&
                   T1,T2,GLOBAL_ODE_REL_ERROR, ATOL,TMP_OUT, CHEM_TIME, WRITE_SUBSTEPS, CVODE_CALL_OPTION)

INITIAL_MIXED_ZN_TMP=TMP_IN_MOD
FINAL_MIXED_ZN_TMP=TMP_OUT
MODIFIED_ZETA=ZETA_IN_MOD
...
! By setting a higher mixed zone temperature, we are allowing the mixed zone equilibrium 
! temperature to be higher than it would have been if it was set at TMP_IN. Hence, Calculate 
! DU/Dt loss rate such that mixed zone temperature will remain under the equilibrium temperture.
SUBROUTINE CALC_DUDT_LOSS(ZZ_UNMIXED,TMP_UNMIXED,ZZ_MIXED,TMP_MIXED,ZETA0,CELL_MASS,DT,DUDT)
   USE PHYSICAL_FUNCTIONS, ONLY : GET_ENTHALPY

   REAL(EB), INTENT(IN)  :: ZZ_UNMIXED(N_TRACKED_SPECIES),ZZ_MIXED(N_TRACKED_SPECIES)
   REAL(EB), INTENT(IN)  :: TMP_UNMIXED,TMP_MIXED,ZETA0,CELL_MASS,DT
   REAL(EB), INTENT(OUT) :: DUDT

   REAL(EB) :: H_UNMIXED, H_MIXED
   REAL(EB) :: DU

   CALL GET_ENTHALPY(ZZ_UNMIXED, H_UNMIXED, TMP_UNMIXED)
   CALL GET_ENTHALPY(ZZ_MIXED,   H_MIXED,   TMP_MIXED)
   DU = (1.0_EB - ZETA0) * CELL_MASS * (H_MIXED - H_UNMIXED)
   DUDT = DU / DT

END SUBROUTINE CALC_DUDT_LOSS
...
USE CHEMCONS, ONLY: I_FUEL,I_CO2,I_H2O,I_O2
...
!> \brief Constant pressure equilibrium flame temperature calculation
!> \param ZZ species mass fraction array
!> \param TMP_IN is the temperature
!> \param AFT is the adiabatic flame temperature (output)
SUBROUTINE CALC_EQUIL_FLAME_TEMPERATURE(ZZ,TMP_IN,PRES_IN,EQUIL_TMP)
USE PHYSICAL_FUNCTIONS, ONLY: CALC_EQUIV_RATIO,GET_ENTHALPY,GET_SPECIFIC_HEAT,GET_TEMPERATURE
REAL(EB), INTENT(INOUT) :: ZZ(N_TRACKED_SPECIES)
REAL(EB), INTENT(IN) :: TMP_IN,PRES_IN
REAL(EB), INTENT(OUT) :: EQUIL_TMP
REAL(EB) :: ZZ_REAC(N_TRACKED_SPECIES),ZZ_PROD(N_TRACKED_SPECIES)

CALL CALC_EQUIL_TMP_REAC_AND_PROD(ZZ,TMP_IN,PRES_IN,EQUIL_TMP,ZZ_REAC,ZZ_PROD)

END SUBROUTINE CALC_EQUIL_FLAME_TEMPERATURE

! Calculate Reactants and products
SUBROUTINE CALC_EQUIL_TMP_REAC_AND_PROD(ZZ,TMP,PRES,TMP_OUT,ZZ_REAC,ZZ_PROD)
USE PHYSICAL_FUNCTIONS, ONLY: CALC_EQUIV_RATIO
USE CHEMCONS, ONLY: I_FUEL,DILUENT_SPECIES_LIST

REAL(EB), INTENT(IN) :: ZZ(N_TRACKED_SPECIES),TMP,PRES
REAL(EB), INTENT(OUT) :: TMP_OUT,ZZ_REAC(N_TRACKED_SPECIES),ZZ_PROD(N_TRACKED_SPECIES)
REAL(EB) :: ZZ_EQUIV(N_TRACKED_SPECIES), ZZ_DILUENTS(N_TRACKED_SPECIES)
REAL(EB) :: PRES_OUT,EQUIV,FUEL_OXIDIZER_MASS,DILUENT_MASS, SUM_ZZ
REAL(EB) :: TOT_OXA,FUEL_OXR,FUEL_OXA
REAL(EB) :: ADDTL_FUEL_REQ
INTEGER :: NS, I
REAL(EB), PARAMETER:: EQUIV_TOL=1.E-4_EB
LOGICAL :: IS_DILUENT
...
IF ((EQUIV - 1.0_EB) > EQUIV_TOL) THEN ! Rich mixures
   ZZ_EQUIV(1:N_TRACKED_SPECIES) = ZZ(1:N_TRACKED_SPECIES)
   ZZ_REAC(1:N_TRACKED_SPECIES) = ZZ(1:N_TRACKED_SPECIES)
   CALL EQUILIBRATE(ZZ_EQUIV,TMP,PRES,TMP_OUT,PRES_OUT,1)
   ZZ_PROD(1:N_TRACKED_SPECIES) = ZZ_EQUIV(1:N_TRACKED_SPECIES)
ELSE  ! Lean mixtures
   ! Set the temperature such that lean zones can burn at stoich, but keep dilution effect
   ZZ_EQUIV = ZZ(1:N_TRACKED_SPECIES)
   ZZ_DILUENTS = 0.0_EB
   ZZ_REAC= 0.0_EB
   DO NS=1, N_TRACKED_SPECIES
      IS_DILUENT = .FALSE.
      IF ((SPECIES_MIXTURE(NS)%OXR > TWENTY_EPSILON_EB .AND. &
             ABS(SPECIES_MIXTURE(NS)%EQUIV - 1.0_EB) < EQUIV_TOL) ) THEN ! DIELUENTS CO2, H2O, SO2 etc.
         IS_DILUENT = .TRUE.
      ENDIF   
      IF(.NOT. IS_DILUENT) THEN 
         DO I = 1, SIZE(DILUENT_SPECIES_LIST)
             IF (TRIM(SPECIES_MIXTURE(NS)%ID) == TRIM(DILUENT_SPECIES_LIST(I))) THEN
                 IS_DILUENT = .TRUE.
                 EXIT
             END IF
         END DO
      ...
      IF (IS_DILUENT) THEN
         ZZ_DILUENTS(NS) = ZZ(NS)
         ZZ_EQUIV(NS) = 0._EB
      ENDIF   
   ENDDO
   FUEL_OXIDIZER_MASS=SUM(ZZ_EQUIV)
   DILUENT_MASS = 1.0_EB - FUEL_OXIDIZER_MASS
   ZZ_EQUIV = ZZ_EQUIV/(FUEL_OXIDIZER_MASS + TWENTY_EPSILON_EB)
   CALL CALC_EQUIV_RATIO(ZZ_EQUIV(1:N_TRACKED_SPECIES), EQUIV)
   IF (SIM_MODE /= DNS_MODE) THEN
      TOT_OXA = 0.0_EB
      FUEL_OXR = SPECIES_MIXTURE(I_FUEL)%OXR
      FUEL_OXA = SPECIES_MIXTURE(I_FUEL)%OXA
      DO NS=1, N_TRACKED_SPECIES
         TOT_OXA = TOT_OXA+ZZ_EQUIV(NS)*SPECIES_MIXTURE(NS)%OXA
      ENDDO
      ADDTL_FUEL_REQ = (1-EQUIV)*TOT_OXA/(FUEL_OXR-FUEL_OXA+TWO_EPSILON_EB) ! To make the mixture stoich
      ZZ_EQUIV(I_FUEL)=ZZ_EQUIV(I_FUEL)+ADDTL_FUEL_REQ
      SUM_ZZ = SUM(ZZ_EQUIV)
      ZZ_EQUIV = ZZ_EQUIV/(SUM_ZZ + TWENTY_EPSILON_EB)
   ENDIF
   
   ! Now add diluents.
   ZZ_EQUIV(1:N_TRACKED_SPECIES) = FUEL_OXIDIZER_MASS*ZZ_EQUIV(1:N_TRACKED_SPECIES) + ZZ_DILUENTS(1:N_TRACKED_SPECIES)
   ZZ_REAC(1:N_TRACKED_SPECIES) = ZZ_EQUIV(1:N_TRACKED_SPECIES)
   CALL EQUILIBRATE(ZZ_EQUIV,TMP,PRES,TMP_OUT,PRES_OUT,1)
   ZZ_PROD(1:N_TRACKED_SPECIES) = ZZ_EQUIV(1:N_TRACKED_SPECIES)
ENDIF

END SUBROUTINE CALC_EQUIL_TMP_REAC_AND_PROD


SUBROUTINE GET_SORTED_SPECIES_INDXS(SORTED_NS, IS_RICH)
   USE CHEMCONS, ONLY: SORTED_SPECIES_OXR,SORTED_SPECIES_OXA
   INTEGER,  INTENT(OUT) :: SORTED_NS(N_TRACKED_SPECIES)
   LOGICAL, INTENT(IN) :: IS_RICH ! 1-Rich (based on OXA) 2-Lean(based on OXR)

   IF (IS_RICH) THEN
      SORTED_NS = SORTED_SPECIES_OXA
   ELSE
      SORTED_NS = SORTED_SPECIES_OXR
   ENDIF   
END SUBROUTINE GET_SORTED_SPECIES_INDXS

SUBROUTINE BUILD_ZZ_UNMIXED_MIXED(ZZ,ZZ_UNMIXED,ZZ_MIXED,ZETA)
   USE PHYSICAL_FUNCTIONS, ONLY: CALC_EQUIV_RATIO
   USE CHEMCONS, ONLY: DILUENT_SPECIES_LIST

   REAL(EB), INTENT(IN)  :: ZZ(N_TRACKED_SPECIES)
   REAL(EB), INTENT(OUT) :: ZZ_UNMIXED(N_TRACKED_SPECIES),ZZ_MIXED(N_TRACKED_SPECIES)
   REAL(EB), INTENT(OUT) :: ZETA ! Unmixed fraction based on ZZ_MIXED and ZZ_UNMIXED
   

   INTEGER  :: SORTED_NS(N_TRACKED_SPECIES)
   INTEGER  :: I, NS

   REAL(EB) :: NUMER, DENOM, NUMER_OLD, DENOM_OLD, PHI, SUM_CHO,EQUIV, Y_REQ_FOR_PHI1, FRAC_TO_REDUCE
   REAL(EB) :: SUM_UNMIXED,SUM_MIXED,ZZ_DILUENTS(N_TRACKED_SPECIES)
   LOGICAL :: IS_DILUENT, IS_RICH
   REAL(EB), PARAMETER:: EQUIV_TOL=1.E-4_EB

   ! Find out diluents and make them separate.
   ZZ_MIXED = 0.0_EB
   ZZ_UNMIXED = ZZ(1:N_TRACKED_SPECIES) 
   ZZ_DILUENTS = 0.0_EB
   ! Separate diluents
   DO NS=1, N_TRACKED_SPECIES
      IS_DILUENT = .FALSE.
   ...
   SUM_CHO = SUM(ZZ_UNMIXED)
   ZZ_UNMIXED =ZZ_UNMIXED/SUM_CHO

   ! Now seaparate mixed and unmixed zone
   IS_RICH = .FALSE.
   CALL CALC_EQUIV_RATIO(ZZ_UNMIXED(1:N_TRACKED_SPECIES), EQUIV)
   IF(EQUIV > 1.0_EB) THEN
      IS_RICH = .TRUE.
   ENDIF      
   CALL GET_SORTED_SPECIES_INDXS(SORTED_NS, IS_RICH)

   NUMER  = 0._EB
   DENOM  = 0._EB
   DO I = 1, N_TRACKED_SPECIES
      NS = SORTED_NS(I)
      NUMER = NUMER + ZZ_UNMIXED(NS) * SPECIES_MIXTURE(NS)%OXR
      DENOM = DENOM + ZZ_UNMIXED(NS) * SPECIES_MIXTURE(NS)%OXA
      ...
      IF ((IS_RICH .AND. PHI <= 1._EB) .OR. (.NOT. IS_RICH .AND. PHI >= 1._EB)) THEN
            ZZ_MIXED(NS) = ZZ_UNMIXED(NS)
            ZZ_UNMIXED(NS) = 0.0_EB
      ELSE
         ! Make PHI=1 and adjust teh species
         NUMER_OLD = NUMER - ZZ_UNMIXED(NS) * SPECIES_MIXTURE(NS)%OXR
         DENOM_OLD = DENOM - ZZ_UNMIXED(NS) * SPECIES_MIXTURE(NS)%OXA

         Y_REQ_FOR_PHI1 = (DENOM_OLD-NUMER_OLD)/(SPECIES_MIXTURE(NS)%OXR-SPECIES_MIXTURE(NS)%OXA)
         IF (Y_REQ_FOR_PHI1 <= 0.0_EB) THEN
            ZZ_MIXED(NS) = ZZ_UNMIXED(NS)
            ZZ_UNMIXED(NS) = 0.0_EB
         ELSE   
            ZZ_MIXED(NS) = Y_REQ_FOR_PHI1
            ZZ_UNMIXED(NS) = ZZ_UNMIXED(NS) - Y_REQ_FOR_PHI1
         ENDIF
         EXIT   ! stop once equivalence ratio is 1
      ENDIF
   ENDDO
   SUM_MIXED= SUM(ZZ_MIXED) ! Fraction of ZZ_UNMIXED
   ZZ_MIXED(1:N_TRACKED_SPECIES) = SUM_CHO*ZZ_MIXED(1:N_TRACKED_SPECIES)+SUM_MIXED*ZZ_DILUENTS(1:N_TRACKED_SPECIES) ! w.r.t total ZZ
   ZZ_UNMIXED(1:N_TRACKED_SPECIES) = SUM_CHO*ZZ_UNMIXED(1:N_TRACKED_SPECIES)+(1-SUM_MIXED)*ZZ_DILUENTS(1:N_TRACKED_SPECIES) ! w.r.t total ZZ

   
   ZETA = 1.0_EB - SUM(ZZ_MIXED)
   IF (ZETA < INITIAL_UNMIXED_FRACTION) THEN
      FRAC_TO_REDUCE = (1-INITIAL_UNMIXED_FRACTION)/(1-ZETA)
      ZETA = INITIAL_UNMIXED_FRACTION
      ZZ_UNMIXED(1:N_TRACKED_SPECIES) = ZZ_UNMIXED(1:N_TRACKED_SPECIES) + (1-FRAC_TO_REDUCE)*ZZ_MIXED(1:N_TRACKED_SPECIES)
      ZZ_MIXED(1:N_TRACKED_SPECIES) = FRAC_TO_REDUCE*ZZ_MIXED(1:N_TRACKED_SPECIES)
   ENDIF

   SUM_MIXED = SUM(ZZ_MIXED)
   ZZ_MIXED=ZZ_MIXED/SUM_MIXED
   
   SUM_UNMIXED = SUM(ZZ_UNMIXED)
   ZZ_UNMIXED=ZZ_UNMIXED/SUM_UNMIXED
   
END SUBROUTINE BUILD_ZZ_UNMIXED_MIXED
...
SUBROUTINE DUMP_CVODE_WARNING_SUMMARY(NCHEM_ACTIVE_CELLS_AND_CC)
   USE CHEMCONS, ONLY: CVODE_WARNING_CELLS,CVODE_ERR_CODE_MIN,CVODE_ERR_CODE_MAX,& 
                       CVODE_WARN_MESSAGES,EQUIL_WARNING_CELLS

   INTEGER, INTENT(IN) :: NCHEM_ACTIVE_CELLS_AND_CC
   INTEGER :: NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL,IERR, CVODE_ERR_CODE, TOT_CVODE_WARN_CELLS,TOT_EQUIL_WARN_CELLS
   REAL(EB) :: PCNT,TNOW
   ...
   IF (N_MPI_PROCESSES > 1) THEN
      ...
      CALL MPI_ALLREDUCE(EQUIL_WARNING_CELLS, TOT_EQUIL_WARN_CELLS, &
                         1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, IERR)
   ELSE
      NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL = NCHEM_ACTIVE_CELLS_AND_CC
   END IF
   TOT_CVODE_WARN_CELLS = SUM(CVODE_WARNING_CELLS)
 
   IF (TOT_CVODE_WARN_CELLS > 0 .AND. NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL > 0 .AND. MY_RANK == 0) THEN
   ...
   IF (TOT_EQUIL_WARN_CELLS > 0 .AND. NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL > 0 .AND. MY_RANK == 0) THEN
      PCNT = REAL(TOT_EQUIL_WARN_CELLS)/REAL(NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL)*100.0_EB
      WRITE(LU_ERR,'(A,I10,A,F6.2,A)')"Total equilibrium warning cells=",TOT_EQUIL_WARN_CELLS,"(",PCNT,"%)"
   ENDIF
   T_CHEM_COMM = T_CHEM_COMM+CURRENT_TIME() - TNOW
...
REAL(EB) :: ZZ(N_TRACKED_SPECIES)
REAL(EB) :: TMP, PRES, EQUIV
REAL(EB) :: TMP_OUT, PRES_OUT
INTEGER :: NS
IF (TRIM(CUSTOM_TEST_TYPE) == 'EQUIL') THEN
   CALL READ_TMP_PRES_ZZ_ARRAY(ZZ, TMP, PRES)
   CALL CALC_EQUIV_RATIO(ZZ,EQUIV)
   WRITE(LU_ERR,*)"EQUIV=",EQUIV
   CALL EQUILIBRATE(ZZ, TMP, PRES, TMP_OUT, PRES_OUT, 1)

   WRITE(LU_ERR,*) 'EQUILIBRIUM calculation complete.'
   WRITE(LU_ERR,*) 'TMP_OUT, PRES_OUT:', TMP_OUT, PRES_OUT
   DO NS = 1, N_TRACKED_SPECIES
      IF (ZZ(NS) > 1.E-6_EB) WRITE(LU_ERR,*)"ID,ZZ=",SPECIES_MIXTURE(NS)%ID,ZZ(NS)
   ENDDO
ELSE
...
SUBROUTINE READ_TMP_PRES_ZZ_ARRAY(ZZ, TMP, PRES)

REAL(EB), INTENT(OUT) :: ZZ(N_TRACKED_SPECIES)
REAL(EB), INTENT(OUT) :: TMP, PRES

INTEGER :: NS, IOS, RANK, COLON_POS
REAL(EB) :: Y_READ
CHARACTER(LEN=256) :: LINE
CHARACTER(LEN=80)  :: READ_LINE_SPECIES
LOGICAL :: FOUND
...
OPEN(UNIT=20, FILE='test_zz.txt', STATUS='OLD', ACTION='READ')

DO
   READ(20,'(A)',IOSTAT=IOS) LINE
   IF (IOS /= 0) EXIT

   WRITE(LU_ERR,*) 'Line=', TRIM(LINE)

   ! ---- TMP / PRES HEADER LINE ----
   IF (INDEX(LINE,'RANK') > 0 .AND. INDEX(LINE,'TMP') > 0) THEN
      ! Read only numeric part
      READ(LINE(INDEX(LINE,':')+1:),*,IOSTAT=IOS) RANK, TMP, PRES
      CYCLE
   END IF
   ...
   ! Read rank from fixed columns (safe)
   READ(LINE(1:10),*,IOSTAT=IOS) RANK
   IF (IOS /= 0) CYCLE

   ! Extract species name
   READ_LINE_SPECIES = ADJUSTL(LINE(11:COLON_POS-1))

   ! Extract mass fraction
   READ(LINE(COLON_POS+1:),*,IOSTAT=IOS) Y_READ
   IF (IOS /= 0) CYCLE

   ! Match species and set ZZ
   FOUND = .FALSE.
   DO NS = 1, N_TRACKED_SPECIES
      IF (TRIM(SPECIES_MIXTURE(NS)%ID) == TRIM(READ_LINE_SPECIES)) THEN
         ZZ(NS) = Y_READ
         FOUND = .TRUE.
         EXIT
      END IF
   ...
   IF (.NOT. FOUND) THEN
      WRITE(LU_ERR,'(A)') 'WARNING: SPECIES NOT FOUND: '//TRIM(READ_LINE_SPECIES)
   END IF
...
WRITE(LU_ERR,*) 'TMP,PRES:', TMP, PRES
DO NS = 1, N_TRACKED_SPECIES
   WRITE(LU_ERR,'(A40,A,ES18.6)') TRIM(SPECIES_MIXTURE(NS)%ID), ':', ZZ(NS)
END DO

END SUBROUTINE READ_TMP_PRES_ZZ_ARRAY
