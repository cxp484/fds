   ...
   IF (NCHEM_ACTIVE_CELLS_AND_CC_GLOBAL > 0) THEN
      ...
      DO INDX=1,NRECEIVE_CELLS
         ...
         CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,&
                                ...
                                MYTEMP,MYRHO,PRES,MYMU,DELTA,VOL,IGN_ZN)
         !***************************************************************************************
      ...
      MESH_LOOP_SERIAL : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         DO NC=1,NCHEM_ACTIVE_CELLS
            ...
            CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q(I,J,K),MIX_TIME(I,J,K),CHI_R(I,J,K),&
                                   ...
                                   LES_FILTER_WIDTH(I,J,K),DX(I)*DY(J)*DZ(K),IGN_ZN,IIC=I,JJC=J,KKC=K )
            !***************************************************************************************
            !IF (STOP_STATUS/=NO_STOP) RETURN
         ...
         IF (CC_IBM) THEN
            ...
            DO NC=1,NCHEM_ACTIVE_CC
               ...
               CALL COMBUSTION_MODEL( T,DT,ZZ_GET,CC%Q(JCC),CC%MIX_TIME(JCC),&
                                      ...
                                      LES_FILTER_WIDTH(I,J,K),CC%VOLUME(JCC),IGN_ZN,IIC=I,JJC=J,KKC=K)
               !***************************************************************************************
...
USE CHEMCONS, ONLY: EQUIV_RATIO_CHECK, MIN_EQUIV_RATIO, MAX_EQUIV_RATIO, N_IGNITION_ZONES, IGNITION_ZONES, &
                    MIN_IGN_ZN_EQUIV_RATIO, MAX_IGN_ZN_EQUIV_RATIO, MIXED_ZN_TMP_FLAG, MIN_MIX_ZN_EQUIV_RATIO, &
                    MAX_MIX_ZN_EQUIV_RATIO
...
IF (.NOT.ALL(REACTION%FAST_CHEMISTRY)) THEN
   ...
   IF (COMBUSTION_ODE_SOLVER==CVODE_SOLVER) THEN 
      IF (N_IGNITION_ZONES > 0) THEN
         ...
         ENDDO
      ENDIF
      IF (IGN_ZN .LE. 0 .AND. SIM_MODE .NE. DNS_MODE .AND. MIXED_ZN_TMP_FLAG) THEN ! LES and other modes. Check equiv ratio
         CALL CALC_EQUIV_RATIO(ZZ_GET(1:N_TRACKED_SPECIES), EQUIV)
         ! No need to solve for no fuel (phi < MIN ) case and only fuel (phi is rich > MAX) case.
         IF (EQUIV .GT. MIN_MIX_ZN_EQUIV_RATIO .AND. EQUIV .LT. MAX_MIX_ZN_EQUIV_RATIO) THEN
...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TMP_IN,RHO_IN,PRES_IN,MU_IN,DELTA,CELL_VOLUME,IGN_ZN,IIC,JJC,KKC)
...
USE CHEMCONS, ONLY: WRITE_CVODE_SUBSTEPS, ZETA_ARTIFICAL_MAX_LIMIT,ZETA_ARTIFICAL_MIN_LIMIT, ZETA_FIRST_STEP_DIV,&
                    IGNITION_ZONES, MIXED_ZN_TMP, MIXED_ZN_TMP_FLAG
...
IF(IGN_ZN > 0) THEN
   ...
   ZETA_IN_MOD = 0.0_EB ! Mix it completely.
ELSE
   IF (SIM_MODE .NE. DNS_MODE .AND. MIXED_ZN_TMP_FLAG) THEN
      TMP_IN_MOD = MAX(TMP_IN,MIXED_ZN_TMP) ! TO DO: Ideal would be a equilibrium temperature based on ZZ(:) and TMP_IN
   ENDIF   
ENDIF
