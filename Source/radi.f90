...
USE RADCONS
USE RADCAL_VAR
USE RADCAL_CALC, ONLY: SUB_RADCAL
USE COMP_FUNCTIONS, ONLY: GET_FILE_NUMBER
USE NNET, ONLY: NNET_MODEL_TYPE,NNET_RADCAL_PL10_N_FEATURES,NNET_RADCAL_PL10_TEMPERATURE,NNET_RADCAL_PL10_X_CO2, &
                NNET_RADCAL_PL10_X_H2O,NNET_RADCAL_PL10_X_CO,NNET_RADCAL_PL10_X_C2H4,NNET_RADCAL_PL10_SOOT_FV, &
                NNET_RADCAL_PL10_MODEL_NAME,NNET_INITIALIZE_NAMED,NNET_EVALUATE
...
REAL(EB), PARAMETER :: KAPPA_DUMP_MINIMUM_TEMPERATURE=400._EB !< Only dump comparison data above this temperature (K)
...
TYPE(NNET_MODEL_TYPE) :: PL10_NN_MODEL                 !< Optional ONNX RadCal 10 cm Planck-mean model
INTEGER :: LU_KAPPA_MODELS = -1
CHARACTER(FILE_LENGTH) :: FN_KAPPA_MODELS
...
MAKE_KAPPA_ARRAYS: IF (.NOT.SOLID_PHASE_ONLY .AND. ANY(SPECIES%RADCAL_ID/='null') .AND. .NOT.WSGG_MODEL) THEN
   ...
   BUILD_KAPPA_ARRAY: IF (N_RADCAL_ARRAY_SIZE>0) THEN
      ...
      ! Direct RadCal retains its spectral work arrays for cell-by-cell calls.
      ! The lookup-table path initializes and releases them during startup.
      ! When dumping, construct the lookup table even if direct RadCal is the
      ! selected model; it is needed for the comparison column.
      IF (RADCAL_DIRECT .AND. .NOT.DUMP_KAPPA_MODELS) THEN
         OMMIN = 50._EB
         OMMAX = 10000._EB
         CALL INIT_RADCAL
      ELSE
            ...
            X_LOOP_Z: DO J=0,N_KAPPA_X-1
               ...
               RADCAL_SPECIES_LOOP: DO NS = 1, N_RADCAL_ARRAY_SIZE
                  ...
                  IF (PATH_LENGTH > TWENTY_EPSILON_EB) THEN
                     ...
                     RADCAL_SPECIES2KAPPA(NS,J+1,K,IBND) = AMEAN/BBF
   ...
   ! Trap any lookup-table errors

   IF (.NOT.RADCAL_DIRECT .OR. DUMP_KAPPA_MODELS) THEN
...
! Initialize the optional RadCal PL10 neural-network model after the FDS to
! RadCal species mapping and condensed-species mask have been constructed.
IF (NNET_PL10 .OR. DUMP_KAPPA_MODELS) THEN
   IF (N_RADCAL_ARRAY_SIZE==0) THEN
      CALL SHUTDOWN('ERROR: NNET_PL10/DUMP_KAPPA_MODELS requires valid RadCal species mapping.')
      RETURN
   ENDIF
   CALL NNET_INITIALIZE_NAMED(PL10_NN_MODEL,NNET_RADCAL_PL10_MODEL_NAME,NNET_IERR,NNET_MESSAGE)
   IF (NNET_IERR/=0) THEN
      CALL SHUTDOWN('ERROR: cannot initialize NNET_PL10 model: '//TRIM(NNET_MESSAGE))
      RETURN
   ENDIF
ENDIF

IF (DUMP_KAPPA_MODELS) THEN
   LU_KAPPA_MODELS = GET_FILE_NUMBER()
   WRITE(FN_KAPPA_MODELS,'(A,A,A,I0,A)') TRIM(RESULTS_DIR),TRIM(CHID),'_kappa_models_rank',MY_RANK,'.csv'
   OPEN(LU_KAPPA_MODELS,FILE=TRIM(FN_KAPPA_MODELS),FORM='FORMATTED',STATUS='REPLACE')
   WRITE(LU_KAPPA_MODELS,'(A)') 'time_s,mesh,i,j,k,temperature_K,kappa_lookup_m_1,kappa_nn_m_1,kappa_direct_m_1'
ENDIF
...
REAL(EB), ALLOCATABLE, DIMENSION(:) :: ZZ_GET,ZZ_KAPPA_DUMP
...
REAL(EB) :: KAPPA_LOOKUP,KAPPA_NN,KAPPA_DIRECT
...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   ELSEIF (N_RADCAL_ARRAY_SIZE>0) THEN
      ...
      !$OMP PARALLEL IF(.NOT.(RADCAL_DIRECT .OR. DUMP_KAPPA_MODELS)) PRIVATE(ZZ_GET,ZZ_KAPPA_DUMP,NNET_IERR,NNET_MESSAGE)
      ALLOCATE(ZZ_GET(1:N_TRACKED_SPECIES))
      IF (DUMP_KAPPA_MODELS) ALLOCATE(ZZ_KAPPA_DUMP(1:N_TRACKED_SPECIES))
      ...
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               ...
               IF (DUMP_KAPPA_MODELS .AND. TMP(I,J,K)>KAPPA_DUMP_MINIMUM_TEMPERATURE) THEN
                  ZZ_KAPPA_DUMP = ZZ_GET
                  KAPPA_LOOKUP = GET_KAPPA(ZZ_KAPPA_DUMP,TMP(I,J,K),IBND)
                  CALL GET_PL10_NN_KAPPA(ZZ_GET,TMP(I,J,K),RHO(I,J,K),KAPPA_NN,NNET_IERR,NNET_MESSAGE)
                  IF (NNET_IERR/=0) CALL SHUTDOWN('ERROR: NNET_PL10 evaluation failed: '//TRIM(NNET_MESSAGE))
                  KAPPA_DIRECT = GET_KAPPA_DIRECT(ZZ_GET,TMP(I,J,K),RHO(I,J,K))
                  WRITE(LU_KAPPA_MODELS,'(ES16.8,4(",",I0),4(",",ES16.8))') T,NM,I,J,K,TMP(I,J,K), &
                     KAPPA_LOOKUP,KAPPA_NN,KAPPA_DIRECT
                  IF (NNET_PL10) THEN
                     KAPPA_GAS(I,J,K) = KAPPA_NN
                  ELSEIF (RADCAL_DIRECT) THEN
                     KAPPA_GAS(I,J,K) = KAPPA_DIRECT
                  ELSE
                     KAPPA_GAS(I,J,K) = KAPPA_LOOKUP
                  ENDIF
               ELSEIF (NNET_PL10) THEN
                  CALL GET_PL10_NN_KAPPA(ZZ_GET,TMP(I,J,K),RHO(I,J,K),KAPPA_GAS(I,J,K),NNET_IERR,NNET_MESSAGE)
                  IF (NNET_IERR/=0) CALL SHUTDOWN('ERROR: NNET_PL10 evaluation failed: '//TRIM(NNET_MESSAGE))
               ELSEIF (RADCAL_DIRECT) THEN
                  KAPPA_GAS(I,J,K) = GET_KAPPA_DIRECT(ZZ_GET,TMP(I,J,K),RHO(I,J,K))
               ELSE
      ...
      ENDDO
      !$OMP END DO
      IF (DUMP_KAPPA_MODELS) DEALLOCATE(ZZ_KAPPA_DUMP)
      DEALLOCATE(ZZ_GET)
      !$OMP END PARALLEL
   ENDIF
...
!> Evaluate the RadCal line-by-line model directly for one gas cell.
!> The direct solver is initialized once in INIT_RADIATION and is called only
!> from a single OpenMP thread because RadCal uses shared work arrays.
REAL(EB) FUNCTION GET_KAPPA_DIRECT(Z_IN,TMP,RHO_GAS)

USE PHYSICAL_FUNCTIONS, ONLY : GET_MOLECULAR_WEIGHT,GET_MASS_FRACTION_ALL
REAL(EB), INTENT(IN) :: Z_IN(1:N_TRACKED_SPECIES),TMP,RHO_GAS
REAL(EB) :: Z_LOCAL(1:N_TRACKED_SPECIES),Y_ALL(1:N_SPECIES),MWA
REAL(EB) :: EFFECTIVE_KAPPA,PLANCK_KAPPA,RADIANCE,TOTAL_TRANSMISSIVITY,GAS_PRESSURE
INTEGER :: N

Z_LOCAL = Z_IN*KAPPA_COND
CALL GET_MOLECULAR_WEIGHT(Z_LOCAL,MWA)
CALL GET_MASS_FRACTION_ALL(Z_LOCAL,Y_ALL)

TEMP_GAS(1) = TMP
TOTAL_PRESSURE_ATM(1) = P_INF/P_STP
PARTIAL_PRESSURES_ATM(:,1) = 0._EB

DO N=1,N_RADCAL_ARRAY_SIZE
   SELECT CASE(RADCAL_SPECIES_INDEX(N))
      CASE(1,2,3,5)
         PARTIAL_PRESSURES_ATM(RADCAL_SPECIES_INDEX(N),1) = TOTAL_PRESSURE_ATM(1)* &
            MAX(0._EB,MWA*DOT_PRODUCT(Y2RADCAL_SPECIES(N,:),Y_ALL))
   END SELECT
ENDDO

IF (SOOT_INDEX>0) THEN
   PARTIAL_PRESSURES_ATM(16,1) = RHO_GAS*MAX(0._EB,Y_ALL(SOOT_INDEX))/SPECIES(SOOT_INDEX)%DENSITY_SOLID
ENDIF
GAS_PRESSURE = PARTIAL_PRESSURES_ATM(1,1) + PARTIAL_PRESSURES_ATM(2,1) + &
               PARTIAL_PRESSURES_ATM(3,1) + PARTIAL_PRESSURES_ATM(5,1)
PARTIAL_PRESSURES_ATM(14,1) = MAX(0._EB,TOTAL_PRESSURE_ATM(1)-GAS_PRESSURE)

CALL SUB_RADCAL(EFFECTIVE_KAPPA,PLANCK_KAPPA,RADIANCE,TOTAL_TRANSMISSIVITY)
IF (PATH_LENGTH > TWENTY_EPSILON_EB) THEN
   !GET_KAPPA_DIRECT = MAX(0._EB,MIN(EFFECTIVE_KAPPA,PLANCK_KAPPA))*100._EB
   GET_KAPPA_DIRECT = MAX(0._EB,EFFECTIVE_KAPPA)*100._EB
ELSE
   GET_KAPPA_DIRECT = MAX(0._EB,PLANCK_KAPPA)*100._EB
ENDIF

END FUNCTION GET_KAPPA_DIRECT
...
USE PHYSICAL_FUNCTIONS, ONLY : GET_MOLECULAR_WEIGHT,GET_MASS_FRACTION_ALL
REAL(EB), INTENT(IN) :: Z_IN(1:N_TRACKED_SPECIES),TMP,RHO_GAS
REAL(EB), INTENT(OUT) :: KAPPA_NN
INTEGER, INTENT(OUT) :: IERR
CHARACTER(LEN=*), INTENT(OUT) :: MESSAGE
REAL(EB) :: Z_LOCAL(1:N_TRACKED_SPECIES),Y_ALL(1:N_SPECIES),MWA,FEATURES(NNET_RADCAL_PL10_N_FEATURES),NN_OUTPUT(1)
INTEGER :: N
...
FEATURES = 0._EB
FEATURES(NNET_RADCAL_PL10_TEMPERATURE) = TMP

DO N=1,N_RADCAL_ARRAY_SIZE
   SELECT CASE(RADCAL_SPECIES_INDEX(N))
      CASE(1)
         FEATURES(NNET_RADCAL_PL10_X_CO2) = MWA*DOT_PRODUCT(Y2RADCAL_SPECIES(N,:),Y_ALL)
      CASE(2)
         FEATURES(NNET_RADCAL_PL10_X_H2O) = MWA*DOT_PRODUCT(Y2RADCAL_SPECIES(N,:),Y_ALL)
      CASE(3)
         FEATURES(NNET_RADCAL_PL10_X_CO) = MWA*DOT_PRODUCT(Y2RADCAL_SPECIES(N,:),Y_ALL)
      CASE(5)
         FEATURES(NNET_RADCAL_PL10_X_C2H4) = MWA*DOT_PRODUCT(Y2RADCAL_SPECIES(N,:),Y_ALL)
   END SELECT
ENDDO

IF (SOOT_INDEX>0) THEN
   FEATURES(NNET_RADCAL_PL10_SOOT_FV) = RHO_GAS*MAX(0._EB,Y_ALL(SOOT_INDEX))/SPECIES(SOOT_INDEX)%DENSITY_SOLID
ENDIF
