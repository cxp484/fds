...
REAL(EB) :: ZETA, MIXING_FACTOR, VOL_CHANGE_TERM, SUM_OMEGA_DOT, SUM_CC, MW0, MW, SUM_H_ZZ0, EXPONENT, &
            CONC, LOSS_FACTOR, RAD_LOSS,MIXED_ZONE_MASS
...
IF(ABS(USER_DATA%DUDT)>0.0_EB) THEN
   MIXED_ZONE_MASS = ZETA*USER_DATA%CELL_MASS
   !WRITE(LU_ERR,*)"Chem.f90 DU/DT, MIXED_ZONE_MASS, LOSS=", &
   !    USER_DATA%DUDT,MIXED_ZONE_MASS,USER_DATA%DUDT/(MIXED_ZONE_MASS*CP+TWO_EPSILON_EB)
   FVEC(N_TRACKED_SPECIES+1)= FVEC(N_TRACKED_SPECIES+1) - USER_DATA%DUDT/(MIXED_ZONE_MASS*CP+TWO_EPSILON_EB)
ENDIF
...
REAL(EB) :: VOL_CHANGE_TERM1, VOL_CHANGE_TERM2, VOL_CHANGE_TERM3, VOL_CHANGE_TERM, SUM_DOMEGA_DOT_BY_DT, MW0, MW, &
            SUM_CP_ZZ0,SUM_H_ZZ0, EXPONENT, CONC, CONC_EXP, DRIDC, DRIDT, LOSS_FACTOR,RAD_LOSS,MIXED_ZONE_MASS,LOSS_TERM
...
IF(ABS(USER_DATA%DUDT)>0.0_EB) THEN
   MIXED_ZONE_MASS = ZETA*USER_DATA%CELL_MASS
   LOSS_TERM = USER_DATA%DUDT/(MIXED_ZONE_MASS*CP+TWO_EPSILON_EB)
   JMAT(N_TRACKED_SPECIES+1, N_TRACKED_SPECIES+1) = JMAT(N_TRACKED_SPECIES+1, N_TRACKED_SPECIES+1) + &
      LOSS_TERM * CPI * DCPDT
   DO NS=1,N_TRACKED_SPECIES
      JMAT(NS, N_TRACKED_SPECIES+1) = JMAT(NS, N_TRACKED_SPECIES+1) - &
               LOSS_TERM * RHOI *(SPECIES_MIXTURE(NS)%MW - CP_I(NS)/CP)
   ENDDO
ENDIF
