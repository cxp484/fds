...
REAL(EB) :: ZETA, MIXING_FACTOR, VOL_CHANGE_TERM, SUM_OMEGA_DOT, SUM_CC, MW0, MW, SUM_H_ZZ0, EXPONENT, &
            CONC, LOSS_FACTOR, RAD_LOSS, LIMITER
...
IF(USER_DATA%USE_DTMPDT_LIMITER) THEN
   LIMITER = 1.0_EB / (1.0_EB + EXP((TMP - USER_DATA%EQUIL_TMP)/USER_DATA%SMOOTH_DELTA))
...
SUBROUTINE CVODE_SERIAL(CC,ZZ_0,TMP_IN,TMP_UNMIX,EQUIL_TMP,USE_DTMPDT_LIMITER,PR_IN,ZETA0,TAU_MIX,CELL_MASS, &
...
REAL(EB), INTENT(IN)    :: ZZ_0(N_TRACKED_SPECIES),TMP_IN,TMP_UNMIX,PR_IN,ZETA0,TAU_MIX,CELL_MASS,TCUR,TEND,EQUIL_TMP
...
SUBROUTINE EQUILIBRATE(ZZ, TMP_IN, PRES_IN, TMP_OUT, PRES_OUT,OPTION)
   ...
   IF(IERR >0) THEN
      ...
      !DO NS = 1,N_TRACKED_SPECIES
      !   WRITE(LU_ERR,'(I4,A40,A,ES18.6)')MY_RANK,TRIM(SPECIES_MIXTURE(NS)%ID),":",ZZ(NS)
      !ENDDO
      !RETURN
   ENDIF
