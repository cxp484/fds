...
SUBROUTINE EQUIL_JACOBIAN(LAMBDA_RT, TARGET_ELEM_MOL_FR, JAC, EQ_OPTION)
   REAL(EB), INTENT(IN)    :: LAMBDA_RT(N_TRACKED_ELEMENTS+1)             ! ITERATION VARIABLES
   ...
   INTEGER :: NE, NS
   INTEGER :: NVAR
   REAL(EB) :: ATOL,LAMBDA_RT_MOD(N_TRACKED_ELEMENTS+1) 
   ...
   !------------------------------------------------------------
   LAMBDA_RT_MOD = LAMBDA_RT
   DO NS = 1, NVAR
     XSAVE = LAMBDA_RT_MOD(NS)
     DX = MAX(ATOL, ABS(XSAVE) * 1.0E-7_EB)

     LAMBDA_RT_MOD(NS) = XSAVE + DX
     CALL EQUIL_RESIDUAL(XX, PRES, LAMBDA_RT_MOD, TARGET_ELEM_MOL_FR, R1, EQ_OPTION)
...
SUBROUTINE SET_INITIAL_MOLES_BASED_ON_STOICHIOMETRY(XX,TMP,PRES)
   ...
   INTEGER  :: NF,NS,ITER, SPECIES_GT_TOL
   INTEGER :: SPECIES_ORDER(N_TRACKED_SPECIES)
   LOGICAL :: REDO, REDO_BASED_ON_SPECIES_TOL
   REAL(EB), PARAMETER:: MOLE_FR_TOL=1.E-6_EB
   ...
   REDO = .TRUE.
   REDO_BASED_ON_SPECIES_TOL = .FALSE.
   ITER = 0
   DO WHILE(REDO)
      CALL COMPUTE_STOICH_MATRIX(XX,STOICH_MAT,SPECIES_ORDER)
      REDO = .FALSE.
      ITER = ITER +1
      IF (ITER > 4) EXIT
      ...
      DO NF = 1, N_FREE
         ...
         DO NS = 1, N_TRACKED_SPECIES
            ...
            IF (NU * DIR < 0.0) THEN
                ...
                IF ((.NOT. REDO .AND. DELTA_XI .LT. 1.0E-10_EB .AND. SPECIES_ORDER(NS) .LE. N_TRACKED_ELEMENTS) &
                  .OR. ITER < 2 ) THEN
                    REDO = .TRUE.
                END IF
         ...
         DO NS = 1, N_TRACKED_SPECIES
            ...
            IF (XX(NS) .GE. MOLE_FR_TOL) SPECIES_GT_TOL = SPECIES_GT_TOL + 1
         END DO
         IF (SPECIES_GT_TOL < N_TRACKED_ELEMENTS) REDO_BASED_ON_SPECIES_TOL = .TRUE. ! Help in element potential
      ENDDO  
...
SUBROUTINE COMPUTE_STOICH_MATRIX(XX,STOICH_MAT,SPECIES_ORDER)
   REAL(EB), INTENT(IN) :: XX(N_TRACKED_SPECIES)
   REAL(EB), INTENT(OUT) :: STOICH_MAT(N_TRACKED_SPECIES,N_TRACKED_SPECIES-N_TRACKED_ELEMENTS) 
   INTEGER,INTENT(INOUT) :: SPECIES_ORDER(N_TRACKED_SPECIES)
