...
! SET ERROR HANDLER
IERR_C = FSUNCONTEXT_PUSHERRHANDLER(SUNCTX, C_FUNLOC(FDS_CVODE_ERR_HANDLER), USERDATAPTR)
IF (IERR_C /= 0) THEN
   WRITE(LU_ERR,*) 'ERROR IN FCVODESETMAXNUMSTEPS, IERR = ', IERR_C, '; HALTING'
   STOP 1
END IF
...
IF (IERR_C /= 0) THEN
   ...
   IF (IERR_C .NE. CV_SUCCESS) THEN
      ...
      IF (DEBUG) THEN
         ...
         DO NS = 1, N_TRACKED_SPECIES
            WRITE(LU_ERR,*)" MY_RANK,ID, Y=",MY_RANK,SPECIES_MIXTURE(NS)%ID, ZZ(NS)
         ENDDO
         WRITE(LU_ERR,*) 'MY_RANK, EQUIVALENCE RATIO, TMP=',MY_RANK, EQUIV, TMP_IN
         WRITE(LU_ERR,*) 'MY_RANK, TMP_UNMIX =', MY_RANK, TMP_UNMIX
         WRITE(LU_ERR,*) 'MY_RANK, PR_IN     =', MY_RANK, PR_IN
         WRITE(LU_ERR,*) 'MY_RANK, ZETA0     =', MY_RANK, ZETA0
         WRITE(LU_ERR,*) 'MY_RANK, TAU_MIX   =', MY_RANK, TAU_MIX
         WRITE(LU_ERR,*) 'MY_RANK, CELL_MASS =', MY_RANK, CELL_MASS
         WRITE(LU_ERR,*) 'MY_RANK, TCUR      =', MY_RANK, TCUR
         WRITE(LU_ERR,*) 'MY_RANK, TEND      =', MY_RANK, TEND
         WRITE(LU_ERR,*) 'MY_RANK, RTOL      =', MY_RANK, RTOL
         WRITE(LU_ERR,*) 'MY_RANK, ATOL      =', MY_RANK, ATOL
