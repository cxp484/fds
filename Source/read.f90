...
USE CHEMCONS, ONLY: ODE_MIN_ATOL, EQUIV_RATIO_CHECK, MIN_EQUIV_RATIO, MAX_EQUIV_RATIO, DO_CHEM_LOAD_BALANCE,FLAME_THICK_FACTOR, &
                    MIXED_ZN_TMP, MIXED_ZN_TMP_FLAG
...
NAMELIST /COMB/ CHECK_REALIZABILITY,COMPUTE_ADIABATIC_FLAME_TEMPERATURE, DO_CHEM_LOAD_BALANCE, EQUIV_RATIO_CHECK, &
                ...
                INITIAL_UNMIXED_FRACTION, MAX_CHEMISTRY_SUBSTEPS, MAX_EQUIV_RATIO, MIN_EQUIV_RATIO, MIXED_ZN_TMP, &
                MIXED_ZN_TMP_FLAG, N_FIXED_CHEMISTRY_SUBSTEPS, ODE_MIN_ATOL,ODE_REL_ERROR,ODE_SOLVER,SUPPRESSION,TAU_CHEM, &
...
IF(TRIM(ODE_SOLVER)=='CVODE') THEN
   ...
   IF (FLAME_THICK_FACTOR > 1._EB) INITIAL_UNMIXED_FRACTION=0.0_EB ! Don't mix thickened flame and batch reaction idea.
   IF (FINITE_RATE_MIN_TEMP<-273._EB) FINITE_RATE_MIN_TEMP=300._EB
   MIXED_ZN_TMP = MIXED_ZN_TMP + TMPM
   COMB_LOOP2: DO
      CALL CHECKREAD('COMB',LU_INPUT,IOS)  ; IF (STOP_STATUS==SETUP_STOP) RETURN
...
ELSE
   FLAME_THICK_FACTOR = 1.0_EB ! Thickened flame model is not effective. 
ENDIF
...
LOGICAL :: PDPA_INTEGRATE,PDPA_NORMALIZE,HISTOGRAM_NORMALIZE,HISTOGRAM,HISTOGRAM_CUMULATIVE,SPARK, IGNITION_ZONE
...
SPARK                    = .FALSE.
IGNITION_ZONE            = .FALSE.
IGNITION_ZONE_TMP        = 726.85_EB !(1000 K)
SPEC_ID                  = 'null'
