...
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,C_MAX,C_MIN,INITIAL_RADIATION_ITERATIONS,KAPPA0,&
                ...
                DUMP_KAPPA_MODELS,NNET_PL10,RADCAL_DIRECT,&
...
IF (NNET_PL10) THEN
   IF (NUMBER_SPECTRAL_BANDS/=1) THEN
      CALL SHUTDOWN('ERROR: NNET_PL10 requires a single gray radiation band.')
      RETURN
   ENDIF
ENDIF

IF (RADCAL_DIRECT .AND. NNET_PL10) THEN
   CALL SHUTDOWN('ERROR: RADCAL_DIRECT and NNET_PL10 cannot both be true.')
   RETURN
ENDIF
IF (RADCAL_DIRECT .AND. NUMBER_SPECTRAL_BANDS/=1) THEN
   CALL SHUTDOWN('ERROR: RADCAL_DIRECT requires a single gray radiation band.')
   RETURN
ENDIF
IF (DUMP_KAPPA_MODELS .AND. NUMBER_SPECTRAL_BANDS/=1) THEN
   CALL SHUTDOWN('ERROR: DUMP_KAPPA_MODELS requires a single gray radiation band.')
   RETURN
ENDIF
