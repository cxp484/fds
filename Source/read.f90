...
USE CHEMCONS, ONLY: ODE_MIN_ATOL,SORTED_SPECIES_OXR,SORTED_SPECIES_OXA
...
INTEGER  :: N_SPEC_READ=0,N1,NN,NNN,NS2,NR,NR2,N_SUB_SPECIES,NS,N_BINS,Y_S,Z_S,POLY_NUM,NE
...
! Populate list of elements in the species, needed for equilibrium calculation
N_TRACKED_ELEMENTS = 0
DO NE=1,MAX_ELEMENTS
   DO NS=1,N_TRACKED_SPECIES
      IF (SPECIES_MIXTURE(NS)%ATOMS(NE) > 0) THEN ! ELEMENT PRESENT IN THE MOLECULE SYMBOL
         N_TRACKED_ELEMENTS= N_TRACKED_ELEMENTS+1
         TRACKED_ELEM_INDX(N_TRACKED_ELEMENTS)=NE
         IF (MY_RANK==0)WRITE(LU_ERR,*) "N_TRACKED_ELEMENTS, ELEM NAME, INDX=",N_TRACKED_ELEMENTS, ELEMENT(NE)%NAME, NE
         EXIT
      ENDIF
   ENDDO   
ENDDO
...
ALLOCATE(SORTED_SPECIES_OXR(N_TRACKED_SPECIES),STAT=IZERO)
ALLOCATE(SORTED_SPECIES_OXA(N_TRACKED_SPECIES),STAT=IZERO)
CALL SORT_SPECIES(SORTED_SPECIES_OXR,.FALSE.)
CALL SORT_SPECIES(SORTED_SPECIES_OXA,.TRUE.)
...
SUBROUTINE SORT_SPECIES(SORTED_NS, IS_RICH)
   ...
   DO NS = 1, N_TRACKED_SPECIES
      IF (IS_RICH) THEN
         KEY(NS) = SPECIES_MIXTURE(NS)%OXA
      ELSE
         KEY(NS) = SPECIES_MIXTURE(NS)%OXR
      ENDIF
      SORTED_NS(NS) = NS
   ENDDO
...
USE CHEMCONS, ONLY: ODE_MIN_ATOL,CVODE_ORDER,EQUIV_RATIO_CHECK, MIN_EQUIV_RATIO, MAX_EQUIV_RATIO, DO_CHEM_LOAD_BALANCE, &
                FUEL_ID_FOR_AFT,MIXED_ZN_MODEL,HEAT_LOSS_FACTOR,RAD_LOSS_FACTOR,MIXING_ZN_TMP_LIMIT,MIXED_ZN_MODEL, &
                ADJUST_ENERGY_FOR_MIXING_ZN
REAL(EB) :: ODE_REL_ERROR
CHARACTER(LABEL_LENGTH) :: RAMP_ZETA_0='null'
NAMELIST /COMB/ ADJUST_ENERGY_FOR_MIXING_ZN,CHECK_REALIZABILITY,COMPUTE_ADIABATIC_FLAME_TEMPERATURE,CVODE_ORDER,CUSTOM_TEST, &
                CUSTOM_TEST_TYPE,DO_CHEM_LOAD_BALANCE,EQUIV_RATIO_CHECK,EXTINCTION_MODEL,FINITE_RATE_MIN_TEMP,FIXED_MIX_TIME,&
                FREE_BURN_TEMPERATURE,FUEL_ID_FOR_AFT,HEAT_LOSS_FACTOR,INITIAL_UNMIXED_FRACTION,MAX_CHEMISTRY_SUBSTEPS,&
                MAX_EQUIV_RATIO,MIN_EQUIV_RATIO,MIXED_ZN_MODEL,MIXING_ZN_TMP_LIMIT,N_FIXED_CHEMISTRY_SUBSTEPS,&
                ODE_MIN_ATOL,ODE_REL_ERROR,ODE_SOLVER,RAD_LOSS_FACTOR,SUPPRESSION,TAU_CHEM,TAU_FLAME,RAMP_ZETA_0,ZZ_MIN_GLOBAL
...
IF(TRIM(ODE_SOLVER)=='CVODE') THEN
   ...
   HEAT_LOSS_FACTOR = MIN(MAX(0.0_EB,HEAT_LOSS_FACTOR),1.0_EB)
   COMB_LOOP2: DO
      CALL CHECKREAD('COMB',LU_INPUT,IOS)  ; IF (STOP_STATUS==SETUP_STOP) RETURN
   ...
   IF(TRIM(MIXED_ZN_MODEL) =="SAME-PHI-KIN-AFT") THEN
      IF (TRIM(FUEL_ID_FOR_AFT) == 'null') THEN
         WRITE(MESSAGE,'(A)') 'ERROR(*): FUEL_ID_FOR_AFT must be specified when USE_MIXED_ZN_TMP is true (default).' &
                     // NEW_LINE('A') // 'Either set USE_MIXED_ZN_TMP=FALSE, or provide FUEL_ID_FOR_AFT in the COMB line.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   ENDIF
   MIXING_ZN_TMP_LIMIT = MIXING_ZN_TMP_LIMIT + TMPM ! Convert to Kelvin
ENDIF
...
REAL(EB) :: SOOT_YIELD,CO_YIELD,HCN_YIELD,EPUMO2,A,LOWER_OXYGEN_LIMIT, &
            ...
            E_LOW_PR,N_T_LOW_PR, A_AD, A_TROE, T1_TROE, T2_TROE, T3_TROE
...
NAMELIST /REAC/ A, A_AD, A_TROE, A_LOW_PR, &
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   SELECT CASE (REACTYPE)
      ...
      CASE('ADSORPTION')
         IF (REVERSE) THEN
            WRITE(LU_ERR,'(A,I0,A)') 'WARN: REAC ',NR,&
                                      ' REVERSE=T is not allowed for ADSORPTION reactions. Setting REVERSE=F.'
            IF (MY_RANK==0) WRITE(LU_ERR,'(A)') TRIM(MESSAGE)
         ENDIF
         RN%REACTYPE = ADSORPTION_TYPE
         RN%A_AD = A_AD
      CASE DEFAULT
...
REAL(EB) :: ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E,CALIBRATION_CONSTANT, &
            ...
            PROBE_DIAMETER,IGNITION_ZONE_TMP,IGNITION_ZONE_UNMIXED_FRACTION
...
NAMELIST /PROP/ ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E,CALIBRATION_CONSTANT,&
                ...
                HISTOGRAM_LIMITS,HISTOGRAM_NBINS,HISTOGRAM_NORMALIZE,ID,IGNITION_ZONE,IGNITION_ZONE_TMP, &
                IGNITION_ZONE_UNMIXED_FRACTION,INITIAL_TEMPERATURE,K_FACTOR,LENGTH,MASS_FLOW_RATE,OFFSET,&
                OPERATING_PRESSURE,ORIFICE_DIAMETER,P0,PARTICLES_PER_SECOND,PARTICLE_VELOCITY,PART_ID,PDPA_END,&
...
READ_PROP_LOOP: DO N=0,N_PROP
   ...
   PY%IGNITION_ZONE_TMP    = IGNITION_ZONE_TMP + TMPM
   PY%IGNITION_ZONE_UNMIXED_FRACTION  = IGNITION_ZONE_UNMIXED_FRACTION
   PY%SPEC_ID              = SPEC_ID
...
SPARK                    = .FALSE.
IGNITION_ZONE            = .FALSE.
IGNITION_ZONE_TMP        = 1226.85_EB !(1500 K)
IGNITION_ZONE_UNMIXED_FRACTION = 0.99
SPEC_ID                  = 'null'
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%PROP_ID /='null') THEN
      ...
      IF (PROPERTY(DV%PROP_INDEX)%IGNITION_ZONE) THEN
         ...
         ENDIF
         IGNITION_ZONES(N_IGNITION_ZONES)%TMP = PROPERTY(DV%PROP_INDEX)%IGNITION_ZONE_TMP
         IGNITION_ZONES(N_IGNITION_ZONES)%ZETA = PROPERTY(DV%PROP_INDEX)%IGNITION_ZONE_UNMIXED_FRACTION
...
CHARACTER(LABEL_LENGTH) :: QUANTITY,SPEC_ID,ELEM_ID,PART_ID,QUANTITY2,PROP_ID,REAC_ID,SLICETYPE
REAL(EB), PARAMETER :: TOL=1.E-10_EB
REAL(FB) :: RLE_MIN, RLE_MAX
TYPE (SLICE_TYPE), POINTER :: SL
NAMELIST /SLCF/ AGL_SLICE,CELL_CENTERED,DB,DRY,ELEM_ID,FYI,DEBUG,ID,MAXIMUM_VALUE,MESH_NUMBER,&
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      VECTORLOOP: DO ITER=1,NITER
         ...
         CALL GET_QUANTITY_INDEX(SL%SMOKEVIEW_LABEL,SL%SMOKEVIEW_BAR_LABEL,SL%INDEX,SL%INDEX2, &
                                 SL%Y_INDEX,SL%Z_INDEX,SL%ELEM_INDEX,SL%PART_INDEX,I_DUM(1),I_DUM(2),SL%REAC_INDEX,SL%MATL_INDEX, &
                                 'SLCF',QUANTITY,QUANTITY2,SPEC_ID,ELEM_ID,PART_ID,'null','null',REAC_ID,'null',-1._EB,I_DUM(3),&
                                 SLICETYPE=SLICETYPE)
...
USE PROPERTY_DATA, ONLY : GET_ELEM_INDEX, ELEMENT
...
QUANTITY_INDEX_LOOP: DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
   ...
   QUANTITY_IF: IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
      ...
      ELSEIF (ELEM_INDEX>0) THEN
         SMOKEVIEW_LABEL = TRIM(ELEMENT(ELEM_INDEX)%NAME)//' '//TRIM(QUANTITY)
         SMOKEVIEW_BAR_LABEL = TRIM(OUTPUT_QUANTITY(ND)%SHORT_NAME)//'_'//TRIM(ELEMENT(ELEM_INDEX)%ABBREVIATION)
