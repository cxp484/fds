...
USE CHEMCONS, ONLY: ODE_MIN_ATOL,CVODE_ORDER,EQUIV_RATIO_CHECK, MIN_EQUIV_RATIO, MAX_EQUIV_RATIO, DO_CHEM_LOAD_BALANCE, &
                FUEL_ID_FOR_AFT,MIXED_ZN_TMP_MODEL,CELL_TMP_MODEL,AFT_EVERYWHERE_MODEL,SOURCE_MIXZN_TMP_MODEL, &
                DTMP_MAX
REAL(EB) :: ODE_REL_ERROR
CHARACTER(LABEL_LENGTH) :: RAMP_ZETA_0='null'
CHARACTER(LABEL_LENGTH) :: MIXED_ZN_TMP='CELL TMP'
NAMELIST /COMB/ CHECK_REALIZABILITY,COMPUTE_ADIABATIC_FLAME_TEMPERATURE,CVODE_ORDER,DO_CHEM_LOAD_BALANCE, &
                DTMP_MAX,EQUIV_RATIO_CHECK,EXTINCTION_MODEL,FINITE_RATE_MIN_TEMP,FIXED_MIX_TIME,FREE_BURN_TEMPERATURE, &
                FUEL_ID_FOR_AFT,INITIAL_UNMIXED_FRACTION, MAX_CHEMISTRY_SUBSTEPS, MAX_EQUIV_RATIO, MIN_EQUIV_RATIO, &
                MIXED_ZN_TMP,N_FIXED_CHEMISTRY_SUBSTEPS, ODE_MIN_ATOL,ODE_REL_ERROR,ODE_SOLVER,SUPPRESSION,TAU_CHEM, &
...
IF(TRIM(ODE_SOLVER)=='CVODE') THEN
   ...
   IF (TRIM(MIXED_ZN_TMP)/='null') THEN
      SELECT CASE (TRIM(MIXED_ZN_TMP))
         CASE ('CELL TMP')
            MIXED_ZN_TMP_MODEL = CELL_TMP_MODEL
         CASE ('AFT EVERYWHERE')
            MIXED_ZN_TMP_MODEL = AFT_EVERYWHERE_MODEL
         CASE ('SRC MIXZN TMP')
            MIXED_ZN_TMP_MODEL = SOURCE_MIXZN_TMP_MODEL   
         CASE DEFAULT
            WRITE(MESSAGE,'(A,A,A)') 'ERROR(188): MIXED_ZN_TMP, ',TRIM(MIXED_ZN_TMP),', is not recognized.'
            CALL SHUTDOWN(MESSAGE) ; RETURN
      END SELECT
   ENDIF   
   ...
   IF(MIXED_ZN_TMP_MODEL == AFT_EVERYWHERE_MODEL .OR. MIXED_ZN_TMP_MODEL == SOURCE_MIXZN_TMP_MODEL) THEN
      IF (TRIM(FUEL_ID_FOR_AFT) == 'null') THEN
         WRITE(MESSAGE,'(A)') 'ERROR(*): FUEL_ID_FOR_AFT must be specified when MIXED_ZN_TMP is set to "AFT EVERYWHERE" or "AFT TMP GRAD".' 
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   ENDIF
ENDIF
...
IGNITION_ZONE_UNMIXED_FRACTION = 0.98
SPEC_ID                  = 'null'
...
USE CHEMCONS, ONLY: N_IGNITION_ZONES,IGNITION_ZONES
