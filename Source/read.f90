...
USE CHEMCONS, ONLY: ODE_MIN_ATOL,CVODE_ORDER,EQUIV_RATIO_CHECK, MIN_EQUIV_RATIO, MAX_EQUIV_RATIO, DO_CHEM_LOAD_BALANCE, &
                FUEL_ID_FOR_AFT,USE_MIXED_ZN_AFT_TMP,MIXING_ZN_TMP_LIMIT,ENABLE_ELEMENT_BALANCE_CHECK
REAL(EB) :: ODE_REL_ERROR
CHARACTER(LABEL_LENGTH) :: RAMP_ZETA_0='null'
NAMELIST /COMB/ CHECK_REALIZABILITY,ENABLE_ELEMENT_BALANCE_CHECK,COMPUTE_ADIABATIC_FLAME_TEMPERATURE,CVODE_ORDER,CUSTOM_TEST, &
                CUSTOM_TEST_TYPE,DO_CHEM_LOAD_BALANCE, EQUIV_RATIO_CHECK, &
                ...
                MIXING_ZN_TMP_LIMIT,N_FIXED_CHEMISTRY_SUBSTEPS, ODE_MIN_ATOL,ODE_REL_ERROR,ODE_SOLVER,SUPPRESSION,TAU_CHEM, &
...
IF(TRIM(ODE_SOLVER)=='CVODE') THEN
   ...
   IF(USE_MIXED_ZN_AFT_TMP) THEN
      ...
      ENDIF
   ENDIF
   MIXING_ZN_TMP_LIMIT = MIXING_ZN_TMP_LIMIT + TMPM
ENDIF
...
USE CHEMCONS, ONLY: YP2ZZ,FUEL_ID_FOR_AFT,I_FUEL,I_CO2,I_CO,I_H2O,I_H2,I_O2
...
IF(TRIM(ODE_SOLVER)=='CVODE') THEN
   ...
   DO NS=1,N_TRACKED_SPECIES
      ...
      ELSEIF(TRIM(SPECIES_MIXTURE(NS)%ID) == "CO2" .OR. TRIM(SPECIES_MIXTURE(NS)%ID) == "CARBON DIOXIDE") THEN
         I_CO2 = NS
      ELSEIF(TRIM(SPECIES_MIXTURE(NS)%ID) == "CO" .OR. TRIM(SPECIES_MIXTURE(NS)%ID) == "CARBON MONOXIDE") THEN
         I_CO = NS
      ELSEIF(TRIM(SPECIES_MIXTURE(NS)%ID) == "H2O" .OR. TRIM(SPECIES_MIXTURE(NS)%ID) == "WATER VAPOR") THEN
         I_H2O = NS
      ELSEIF(TRIM(SPECIES_MIXTURE(NS)%ID) == "H2" .OR. TRIM(SPECIES_MIXTURE(NS)%ID) == "HYDROGEN") THEN
         I_H2 = NS
      ENDIF
   ENDDO
