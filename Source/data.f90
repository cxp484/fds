...
USE PRECISION_PARAMETERS
USE THERMO_PROPS
USE GLOBAL_CONSTANTS, ONLY: N_ELEMENTS
IMPLICIT NONE (TYPE,EXTERNAL)
...
TYPE(ELEMENT_TYPE):: ELEMENT(N_ELEMENTS)
...
USE GLOBAL_CONSTANTS, ONLY: PR_T,N_ELEMENTS
...
REAL(EB) :: SIGMAIN,EPSOKIN,MWIN,ATOM_COUNTS(N_ELEMENTS),H_FIN,PR_GASIN
...
USE GLOBAL_CONSTANTS,ONLY:MW_N2,MW_O2,MW_CO2,MW_H2O,MW_CO,MW_H2,MW_HCN,N_ELEMENTS
REAL(EB)::ATOM_COUNTS(N_ELEMENTS)
...
USE GLOBAL_CONSTANTS,ONLY:LU_ERR,N_ELEMENTS
...
REAL(EB),INTENT(INOUT)::ATOM_COUNTS(N_ELEMENTS)
REAL(EB)::SUB_WEIGHT,NUM
INTEGER :: I,J,LEN1,ELEMENT_INDEX
REAL(EB):: SUBATOM_COUNTS(N_ELEMENTS)
...
DO WHILE (I <= LEN1)
   ...
   ELSE SUBUNIT_SEARCH
      ...
      ENDIF
      ELEMENT_INDEX = -1
      ELEMENT_SEARCH: DO J=1,N_ELEMENTS
...
USE GLOBAL_CONSTANTS, ONLY: LU_ERR,FN_OUTPUT,LU_OUTPUT,CHID,N_SPECIES,N_TRACKED_SPECIES,MY_RANK,N_ELEMENTS
USE TYPES, ONLY:SPECIES,SPECIES_TYPE,SPECIES_MIXTURE_TYPE,SPECIES_MIXTURE,REACTION_TYPE,REACTION
USE COMP_FUNCTIONS, ONLY:SHUTDOWN
INTEGER, INTENT(IN) :: NR
INTEGER :: I,NN,NS
REAL(EB), INTENT(IN) :: ATOM_COUNT(N_ELEMENTS),ERR
REAL(EB) :: REACTION_BALANCE(N_ELEMENTS)
LOGICAL :: EX
